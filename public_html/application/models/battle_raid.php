...
class Battle_Raid_ModelBattleType extends Model_BattleType
{
	var $battletype = 'raid';
	var $par = null;
	var $attackers = array();
	...
	*/
	...
	{		
		...
		$this -> battlereport[]['battleround'] = 
			'__battle.raidintroduction' . ';'  . '__' . $this -> attackingregion -> name . ';' . '__' . $this -> attackedregion -> name . ';' . Model_Utility::format_datetime( time() );
	...
	*/
	...
	function do_aftermath()
	{
		...
		if ( $winners == 'attackers' )
		{
			...
			// Strutture distrurre � direttamente proporzionale
			...
			foreach ($raidablestructures as $raidablestructure)			
			{
				...
				{							
					...
					if ( $totalweight > $itemtotalweight )
					{
						...
						// se la struttura � mercato, evento al seller
