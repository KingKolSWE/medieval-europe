...
<?php
	...
	{	
		...
		// malattie e salute -- solo se ha un ruolo religioso o intel >= 18
		
		if (
			$viewingchar -> position_id == $character -> position_id and					
			(
				(!is_null($viewingcharrole) and $viewingchar -> has_religious_role() )
				or 
		...
		)
		{
			...
			foreach ( (array) $diseases as $disease )
			{
				$title = kohana::lang('character.' . $disease -> param1 . '_diseasebadgedesc').
					"<br/><br/>" .		
					html::anchor(
						'character/cure/disease/' . $disease -> param1 . '/' . $character -> id, 
