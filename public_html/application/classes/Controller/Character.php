...
class Controller_Character extends Controller_Template
{
	...
	*/
	...
	{
		KO7::$log->add(KO7_Log::INFO, '-> character.php create');

		$view = View::factory('character/create');
		...
		$view -> combo_sex = array('M'=>__('character.create_male'), 'F'=>__('character.create_female'));
		...
		if ( $_POST )
		{
			...
			if ($post -> validate() )
			{
				...
				//KO7::$log->add(KO7_Log::DEBUG, 'region: ' . $region -> name ); exit();
				...
				try
				{
					...
					Model_Character::assign_tutor( $char, $language1 -> language);
					KO7::$log->add(KO7_Log::INFO, '-> createchar ***commit***.');
					$db -> query("commit");
				...
				{
					KO7::$log->add(KO7_Log::ERROR, kohana::debug( $e->getMessage() ));
				...
				Model_Character::modify_stat_d($char -> user_id, 'speedbonus', 100, null, null, true );
		...
		KO7::$log->add(KO7_Log::INFO, '-> character.php end create');
	}
	...
	*/
	...
	public function details()
	{
		...
		$view    = View::factory ('character/details');
		...
		if ( $char -> is_sick() )
		{
			...
			{
				$diseasesarray[] = __('character.disease_' . $disease -> param1 );
			}
		}

		$stat = Model_Character::get_stat_d( $char -> id, 'honorpoints');
			if (!$stat -> loaded)
				$view -> honorpoints = 0;
			else
		...
		$intoxicationlevel_stat = Model_Character::get_stat_d( $char -> id,	'intoxicationlevel' );
		...
		$view -> intoxicationlevel = __('global.' . $intoxicationlevel . '_f');

		$skills = $char -> get_stats('skill');
		$view -> skills = $skills;
		// religione
		$view -> faithlevel = Model_Character::get_stat_d( $char -> id, 'faithlevel' );
		$view -> afp = Model_Character::get_stat_d( $char -> id, 'fpcontribution', $char -> church_id );
		$view -> alms = Model_Character::get_stat_d( $char -> id, 'alms', $char -> church_id );
	...
	*/
	...
	public function inventory()
	{
		...
		$view    = View::factory('character/inventory');
		$sheets  = array('gamelayout' => 'screen', 'character'=>'screen', 'pagination'=>'screen', 'submenu'=>'screen');
		...
		$items = Model_Character::inventory( $character -> id );
		$equippeditems = Model_Character::get_equipment( $character -> id );
		...
		//KO7::$log->add(KO7_Log::DEBUG, kohana::debug($partinfo));
	...
	*/
	...
	{
		$view    = View::factory('character/role');
		$sheets  = array('gamelayout' => 'screen', 'character'=>'screen', 'pagination'=> 'screen', 'submenu'=>'screen');
		$char = Model_Character::get_info( Session::instance()->get('char_id') );
	...
	*/
	function publicprofile( $character_id = null )
	{
		...
		$view = View::factory('character/publicprofile_' . $character -> type);
		...
		$stat = Model_Character::get_stat_d( $character -> id, 'honorpoints');
		if (!$stat -> loaded)
			$view -> honorpoints = 0;
		else
			$view -> honorpoints = $stat -> value;
		...
		$header = View::factory( 'character/profileheader_' . $character->type );
	...
	*/
	...
	{
		$view = View::factory( 'character/change_description' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
	...
	*/
	...
	{
		$view = View::factory( 'character/change_avatar' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		...
		else
		{
			...
			if ( $files -> validate() )
			{
				...
				//KO7::$log->add(KO7_Log::DEBUG, kohana::debug($image) );
	...
	*/
	...
	function changeregion()
	{
		...
		$char = Model_Character::get_info( Session::instance()->get('char_id') );
		$view = View::factory( 'character/changeregion');
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
	...
	*/
	...
	function listall()
	{
		...
		$criteria = __('global.criteria' );

		if ( $name != '' )
		{
			$sql .= " and c.name like '%" . $name . "%' " ;
			$criteria .= __('global.name') . ' ' . __('global.contains') . ' ' . $name . ' ' ;
		}

		if ( $online )
		{
			$sql .= " and c.lastactiontime > (unix_timestamp() - " . Kohana::config('medeur.maxidletime') . " ) " ;
			$criteria .= __('global.online') . ' = true' ;
		}

		if ( !$online and $name == '' )
			$criteria .= __('global.allrecords' ) ;
	...
	*/
	...
	{
		$view = View::factory( 'character/change_slogan' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
	...
	*/
	...
	public function history( $character_id )
	{
		...
		$view = View::factory( 'character/history' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen', 'character' => 'screen');
	...
	*/
	...
	{
		$view = View::factory( 'character/change_history' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		$character = Model_Character::get_info( Session::instance()->get('char_id') );
	...
	*/
	...
	{
		$view = View::factory( 'character/change_attributes' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
	...
	*/
	...
	public function complete_action( $charflag = false )
	{
		...
		KO7::$log->add(KO7_Log::INFO, "-> *** Completed $nactions actions. Elapsed:" .  ( $end - $start ) . ' seconds. actions per sec:' . ( $nactions / max(1, ($end - $start)) ) );
	...
	*/
	...
	public function accessrpforum()
	{
		...
		$view = View::factory( 'character/accessrpforum' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		$char = Model_Character::get_info( Session::instance()->get('char_id') );
		KO7::$log->add(KO7_Log::DEBUG, '-> Rpforumregistered: ' . $char -> rpforumregistered );
		$data = array(
			'username' => '',
			'password' => ''
		);
  ...
  public function _c_acceptrpforumtos(Validation $array, $field)
  {
	...
	*/
	...
	public function myproperties()
	{
		...
		$view    = View::factory ('character/myproperties');
	...
	*/
	...
	public function myjobs( )
	{
		...
		$char = Model_Character::get_info( Session::instance()->get('char_id') );
		$view    = View::factory ('character/myjobs');
		$sheets  = array('gamelayout' => 'screen', 'character'=>'screen', 'pagination'=>'screen', 'submenu'=>'screen');
		$limit = 10;
	...
	*/
	...
	{
		$view = View::factory( 'character/change_signature' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
	...
	*/
	...
	function myquests()
	{
		$view = View::factory( 'character/myquests' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		$character = Model_Character::get_info( Session::instance()->get('char_id') );
		$questreport = array();
	...
	*/
	...
	public function launchduel( $targetchar_id )
	{
		...
		$view = View::factory( 'character/launchduel' );
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		$character = Model_Character::get_info( Session::instance()->get('char_id') );
	...
	*/
	...
	function acceptweddingproposal( $response, $id )
	{
		...
		if ($proposal -> loaded )
		{
			...
			if ( $response == 1 )
			{
				...
				';' . Model_Character::create_publicprofilelink($proposal -> tochar_id, null),
				'normal' );
			}
			elseif ( $response == 0 )
			{
	...
	function glance( $glanceedchar_id )
	{
		...
		if ( $ca -> do_action( $par,  $message ) )
		{
			$view = View::factory( 'character/glance' );
			$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
	...
	*/
	...
	function loot( $character_id = null )
	{
		...
		$view = View::factory ('character/targetinventory');
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		$character = Character_Model::get_info( Session::instance() -> get('char_id') );
	...
	*/
	...
	{
		// Char che battezza
		$character_source = Model_Character::get_info( Session::instance()->get('char_id') );
