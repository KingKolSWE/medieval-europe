...
class Controller_Affiliate extends Controller_Template
{		
	
		
	function register()
	{
		$view = View::factory('affiliates/register');
	...
	function login()
	{
		...
		$view = View::factory ('affiliates/login');
		$sheets  = array( 'affiliates' => 'screen' );
		$this -> template = View::factory('template/affiliates');
		...
		if ( $this -> input -> post() )
		{
			...
			if ($post -> validate() )
			{
				...
				if ( is_null( $error ) )
				{
					...
					KO7::$log->add(KO7_Log::DEBUG, "User: {$user}, Pass: {$password}");
	...
	{		
		...
		$view = View::factory ('affiliates/forgotpassword');
		$sheets  = array( 'affiliates' => 'screen' );
		$this -> template = View::factory('template/affiliates');
	...
	public function dashboard()
	{
		...
		$view = View::factory ('affiliates/dashboard');
		$sheets  = array( 'affiliates' => 'screen' );
		$this -> template = View::factory('template/affiliates');
	...
	{
		$view = View::factory ('affiliates/statistics');
		$sheets  = array( 'affiliates' => 'screen' );
		$this -> template = View::factory('template/affiliates');
		$user = Auth::instance() -> get_user();
	...
	public function media()
	{
		...
		$view = View::factory ('affiliates/media');
		$sheets  = array( 'affiliates' => 'screen' );
		$this -> template = View::factory('template/affiliates');
		$this->template->sheets = $sheets;   		
		$user = Auth::instance() -> get_user();
	...
	public function logout()
	{
		...
		$view = View::factory ('affiliates/logout');
		$sheets  = array( 'affiliates' => 'screen' );
		$this -> template = View::factory('template/affiliates');
		$this->template->sheets = $sheets;   		
