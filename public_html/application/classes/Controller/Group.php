...
class Controller_Group extends Controller_Template
{
	...
	*/
	...
	function listall()
	{
		...
		$view    = View::factory ('group/listall');
	...
	*/	
	...
	function mygroups()
	{
		...
		$view -> submenu = $submenu;
		$view->character = $char;
		$view -> groups = Model_Character::get_info( Session::instance() -> get('char_id') ) -> get_my_groups();
		$this->template->content = $view;
		$this->template->sheets = $sheets;
	}
	...
	*/
	...
	function create()
	{
		...
		// Controllo che il char non abbia già 3 gruppi (max consentito)
		...
		$view -> combo_type = array(
			'mercenary' => __('groups.mercenary'), 
			'military' => __('groups.military'), 
			'other' => __('groups.other')
		);
		
		// Inizializzo la combo per il campo secret
		
		$view -> combo_secret = array('1'=>__('global.yes'), '0'=>__('global.no'));
	...
	// Funzione: Callback di validazione per verificare se il nome � già stato preso
	...
	*/
	...
	function upload_image ($group_id)
	{
		...
		$view = View::factory( 'group/upload_image' );
		$sheets = array(
			'gamelayout' => 'screen', 
			'character'=>'screen', 
			'pagination'=>'screen', 
	...
	*/
	...
	function view( $group_id )
	{
		...
		$limit = 15;
		$view = View::factory( 'group/view' );
		$subm  = View::factory ('template/submenu');
		$sheets = array('gamelayout' => 'screen', 'character'=>'screen', 'pagination'=>'screen', 'submenu'=>'screen');		
		$group = ORM::factory('group', $group_id);
		$character = Model_Character::get_info( Session::instance()->get('char_id') );
		$istutor = Model_Character::has_merole($character, 'newborntutor');
	...
	*/
	...
	function delete ($group_id)
	{
		...
		$owner = Model_Character::get_info( Session::instance()->get('char_id') );
		$group = ORM::factory('group', $group_id);
		...
		{
			// Se l'utente non � pendente invio una notifica
			if ($member->joined)
			{
	...
	*/
	...
	{	
		...
		$view = View::factory( 'group/message' );
		$sheets = array('gamelayout' => 'screen', 'character'=>'screen', 'pagination'=>'screen', 'submenu'=>'screen');	
	...
	*/
	...
	function transfer_leadership ( $group_id = null)
	{
		...
		$char  = Model_Character::get_info( Session::instance()->get('char_id') );
		$view = View::factory( 'group/transfer_leadership' );
		$sheets  = array('gamelayout' => 'screen', 'character'=>'screen', 'pagination'=>'screen', 'submenu'=>'screen');
		$form = array ('group_charname' => '');		
		$errors = $form;
		...
		else
		{
			...
			if ($post->validate() )
			{
				...
				foreach ( $members as $member )
				{
					...
					$body = __('groups.transfered_leadership', $group -> name, $newleader -> name ); 
	...
	*/
	...
	function leave( $group_id )
	{
		...
		$subject = __('groups.massive_message_subject', $group -> name);
		$body = __('groups.charleftgroup', $char -> name ); 
		$ret = $m -> send( $sender, $recipient, $subject, $body );
