...
class Controller_Toplist extends Controller_Template
{
	...
	*/
	...
	function vote( $type, $toplist_id = null )
	{
		...
		$view = View::factory ( '/toplist/sitelist' ); 
		$sheets  = array('gamelayout'=>'screen', 'submenu'=>'screen');
		$db = Database::instance();
		$char = Model_Character::get_info( Session::instance()->get('char_id') );
		$vt = array();	
		$playercanvote = true;
		KO7::$log->add(KO7_Log::INFO, '-> char: ' . $char -> name . ' is voting for toplist: ' . $toplist_id );
		...
		$stat = Model_Character::get_stat_d($char -> id, 'dailytoplistvotes', $type );
		if ( $stat -> loaded )
		{
		...
		else
		{
			...
			KO7::$log->add(KO7_Log::INFO, '-> char: ' . $char -> name . ' is voting for toplist: ' . $toplist_id . ' - redirecting to toplist...' );
			$par[0] = $char;
			$par[1] = $toplist_id;
	...
	*/
	...
	function obs_reward_arenatop100( )
	{
		...
		KO7::$log->add(KO7_Log::INFO, '-> userId' . $userId );
		KO7::$log->add(KO7_Log::INFO, '-> userip' . $userIp );
		KO7::$log->add(KO7_Log::INFO, '-> valid' . $valid );
		KO7::$log->add(KO7_Log::INFO, '-> at_refc' . $at_refc );
	...
	*/
	...
	function reward()
	{
		...
		KO7::$log->add(KO7_Log::INFO, "-> Topsite: receiving a reward callback --- ");
		KO7::$log->add(KO7_Log::INFO, "-> Topsite: key: $key");
		...
		if ( $key == '' )
		{
			KO7::$log->add(KO7_Log::ERROR, '-> Toplist callback: no parameter received, exiting.');
			exit();
		}
