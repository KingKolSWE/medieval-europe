...
class Controller_Batch extends Controller_Template
{
	...
	{
		KO7::$log->add(KO7_Log::DEBUG, "------- MERGING REGIONS: {$regionname1} {$regionname2} in: {$regionname2}, saving resource in {$resourcetomantain}, using clima of: {$climatosave}, dry run: {$simulate}");
		...
		$characters_plague_delete = Database::instance() -> query( " delete from character_stats where name ='disease' and param1='plague'; ");
				...
				foreach ( $characters_kingdom  as $row)
				{
					...
					$temp = Database::instance() -> query("select c.id from characters c
						...
						KO7::$log->add(KO7_Log::DEBUG, ('limite di infettati di '. $row -> count .' per il regno ' . $row -> kingdom_id .' ' ));
