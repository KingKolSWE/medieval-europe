...
class Model_CharacterAction_CADamage extends Model_CharacterAction
{
	...
	{ 
		...
		// check: non � possibile danneggiare una struttura se esistono delle
		// strutture figlie
		...
		// check: non � possibile danneggiare una struttura se la diplomazia del regno della struttura � ostile 
		...
		// check: il char � nella stessa regione della struttura
	...
	public function complete_action( $data )	{	
		...
		$structure = Model_StructureFactory::create( null, $data -> param1 );
		
		// Consumo degli items/vestiti indossati
		Model_Item::consume_equipment( $this->equipment, $character, $data->param2 );
				
		// decrementa la qualit� della struttura		
		if ( $structure -> loaded )
		{
		...
		// decremente il livello di fede solo se il char � della stessa religione
		if ( $structure -> structure_type -> church_id == $character -> church_id )
			$character -> modify_faithlevel( $data -> param2 * -15 );
		
		// decremente il numero di AFP per ogni danneggiamento se il char � della stessa religione
		if ( $structure -> structure_type -> church_id == $character -> church_id )
			$character -> modify_stat( 
				'fpcontribution', $data -> param2 * -25, $character -> church_id );

		// aumenta il livello di fede solo se il char � di diversa religione	
		...
		// aumenta il numero di AFP solo se il char � di diversa religione	
		if ( $structure -> structure_type -> church_id != $character -> church_id and $character -> church -> name != 'nochurch' )
			$character -> modify_stat( 
				'fpcontribution', $data -> param2 * 5, $character -> church_id );			

		// add a structure event
		Model_StructureEvent::newadd( $structure -> id,
		...
		// se la condizione della struttura � < 0, distruggila		
		// distruggi anche tutti le strutture figlie se in costruzione
		
		if ( $structure -> state <= 0 )
		{
			// solo se non � un cantiere, rimuovi il ruolo
