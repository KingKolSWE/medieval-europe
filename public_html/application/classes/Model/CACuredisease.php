...
class Model_CharacterAction_CACuredisease extends Model_CharacterAction
{
	...
	{ 
		...
		$has_dogma_bonus = Model_Church::has_dogma_bonus($par[0] -> church_id, 'curediseaseextended');
		...
		// Check: il char che cura non ha saziet� sufficiente
		if
		(
		...
		// Check: il char che viene curato � impegnato in un'altra azione
		$pendingaction = Model_CharacterAction::get_pending_action( $par[1] );
		...
		$fp = Model_Structure::get_stat_d( $this -> controlledstructure -> id, 'faithpoints' );
		...
		$this -> disease = Model_DiseaseFactory::createDisease($par[2]);
		...
		// Check: il sacerdote � di livello 1,2 o 3
		...
		// Check: il char che deve essere curato � ateo 
		// Check: la chiesa non ha il bonus dogma
		if 
		(
		...
		// Check: il personaggio da curare non � ateo
	...
	*/
	protected function append_action( $par, &$message )
	{
		...
		// solo se il prete non sta curando s� stesso
		/////////////////////////////////////////////////
		...
		/////////////////////////////////////////////////
		// Salva una action blocking per chi � curato		
		/////////////////////////////////////////////////
		...
		{		
			$i = Model_Item::factory( null, 'medicalkit' );
		...
		Model_MyCache::delete(  '-charinfo_' . $par[0] -> id . '_currentpendingaction');
		
		// Rimuove l'infuso necessario alla cura dall'inventario del char
		
		$required_item = $this -> disease -> get_requireditem();
		$i = Model_Item::factory( null, $required_item );
		$i -> removeitem( "character", $par[0]->id, 1 );
	...
	*/
	public function complete_action( $data )
	{
		...
		// Char a cui � legata l'azione da completare
		...
		// Char che � stato curato
		$chartarget = ORM::factory('character', $data -> param2 );
		// Struttura che � controllata dal curante
		...
		$has_dogma_bonus = Model_Church::has_dogma_bonus($charsource -> church_id, 'curediseaseextended');
		
		if ( $charaction -> id == $charsource -> id )
		{
						
			// Consumo degli items/vestiti indossati
			Model_Item::consume_equipment( $this->equipment, $charsource );
		...
		// ***************************************
		// Azioni relative a chi � stato curato
		// ***************************************
		
		if ( $charaction -> id == $chartarget -> id )
		{		
						
			$disease = Model_DiseaseFactory::createDisease( $data -> param4);
			$disease -> cure_disease( $chartarget );
			
			if ($data -> param5 == true )
			{
				// sempre true perch� basta avere il bonus disease base
				$hptorestore = Model_CharacterAction_CACurehealth::get_hptorestore($chartarget, true);
	...
	public function cancel_action() { 
		...
		if ($this -> character_id == $targetchar -> id )
		{
			...
			Model_MyCache::delete(  '-charinfo_' . $sourcechar -> id . '_currentpendingaction' );
		...
		if ($this -> character_id == $sourcechar -> id )
		{
			...
			Model_MyCache::delete(  '-charinfo_' . $targetchar -> id . '_currentpendingaction' );
		}
	...
	* @param Character_Model $stargetcharacter personaggio che � curato
