...
class Model_CharacterAction_CAImprison extends Model_CharacterAction
{
	...
	*/
	...
	{ 
		...
		$judgekingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[0] -> region -> kingdom_id, 'running');
		$criminalkingdomrunningwars = Model_Kingdom::get_kingdomwars(  $par[1] -> character -> region -> kingdom_id, 'running');
		...
		$runningbattles = Model_Kingdom::get_runningbattles( $par[0] -> region -> kingdom_id );
	...
	{	
		...
		$pendingaction = Model_CharacterAction::get_pending_action( $par[1] -> character_id );
		...
		$distance = Model_RegionPath::compute_distance(
			$par[4] -> region -> name, 
			$par[3] -> region -> name);				
		...
		$a =  Model_CharacterAction::factory("imprison");
		...
		Model_CharacterEvent::addrecord(
			...
			Model_Utility::format_datetime( time() + $par[2] * 3600 )
			);		
	...
	{ 
		...
		if ( $char -> loaded )
		{
			...
			$distance = Model_RegionPath::compute_distance( $sourceregion -> name, $targetregion -> name);			$glut = min( 50, round( self::GLUT_FOR_30_MIN * ( $distance / 30 ) ) ) ;
