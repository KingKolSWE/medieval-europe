...
{	
	...
	{		
		...
		Model_MyCache::delete (  '-charinfo_' . $r -> id . '_unreadmessages' );
	...
	else
	{
		...
		Model_Utility::send_notification( $recipient -> user -> id, "You received an Ingame message: {$subject}", nl2br($body));
		
		Model_MyCache::delete (  '-charinfo_' . $recipient -> id . '_unreadmessages' );
