...
class Model_CharacterAction_CABuystructure extends Model_CharacterAction
{
	...
	{ 
		...
		$structureinstance = Model_StructureFactory::create( $par[0] );
		
		$this -> price = $structureinstance -> getPrice( $par[1], $par[2]);
		$this -> baseprice =  $structureinstance -> getBaseprice();
		
		// Se � una regione indipendente, non � possibile
		// comprare strutture
		...
		// se il char � ostile, non pu� comprare propriet�
    ...
    if ( $structureinstance -> structure_type -> parenttype == 'shop' )
		{
			...
			// se ha gi� un numero di negozi > di quelli permessi errore.
        ...
        return false;
			...
			else if ($ownedshops == Kohana::config('medeur.maxshops'))		
			{
				...
				$churchhasoraetlaborabonus = Model_Church::has_dogma_bonus( $par[1] -> church_id, 'craftblessing');
		...
		// verifica se il char ha gi� case in questo nodo
	...
	{	
		...
		if ( $structureinstance -> structure_type -> parenttype == 'shop' )		
		{
			$license = Model_Item::factory( null, 'scroll_propertylicense' );
		...
		$net = Model_TaxValueAdded::apply( $_par );
						
		// eventi		
		
		Model_CharacterEvent::addrecord( $par[1] -> id, 'normal', '__events.charstructurebought' .
