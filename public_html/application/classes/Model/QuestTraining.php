...
class Model_TrainingQuest extends Model_Quest
{
	...
	function activate( $character, &$message, $spare3 = null, $spare4 = null )
	{
		...
		$quest = Model_QuestFactory::createQuest('cultivatecrops');
		...
		if ( $character -> sex == 'M' )
			$item = Model_Item::factory( null, 'shoesm_1' );
		else
			$item = Model_Item::factory( null, 'shoesf_1' );
		$item -> quality = 40;
		...
		$item = Model_Item::factory( null, 'writingkit' );
		$item -> quality = 10;
		$item -> additem( 'character', $character -> id , 1 );
		
		$item = Model_Item::factory( null, 'woodensword' );
		$item -> quality = 10;
		$item -> additem( 'character', $character -> id , 1 );
