...
class Model_CharacterAction_CAStartnativerevolt extends Model_CharacterAction
{
	...
	// In questo caso l'azione � di tipo immediato e quindi la funzione non viene evocata
	...
	public function complete_action ( $data ) 
	{
		...
		foreach ( $regions as $region )
		{
			...
			******************************************/
			...
			// Se il regno a cui appartiene la regione � stato attaccato
			...
			******************************************/
			...
			// Determiniamo se il regno � in guerra.
			// LOG
			...
			{				
				$data = array();
				$iskingdomfighting = Model_Kingdom::is_fighting( $region -> kingdom_id, $data );
				
				// solo se � in guerra, non valutare più le altre regioni.
			...
			// Se il regno � in guerra e si sta difendendo, score negativo
			...
			// Se per la regione c'� qualche attacco in corso, score negativo
			...
			if ( isset( $capitals[$region -> kingdom_name] ) )
				...
				$capitals[$region -> kingdom_name] = Model_Kingdom::get_capitalregion($region -> kingdom_id);
			
			$capital = $capitals[$region -> kingdom_name];				
			$distance = Model_RegionPath::compute_distance( $capital -> name, $region -> region_name);
			// LOG
			...
			if ( isset( $_totalkingdomresidents[$region -> kingdom_name] ) )
				...
				$_totalkingdomresidents[$region -> kingdom_name] = Model_Kingdom::get_citizens_count( $region -> kingdom_id );
			...
			if ( isset( $_totalresidentsinkingdom[$region -> kingdom_name] ) )
				...
				$_totalresidentsinkingdom[$region -> kingdom_name] = Model_Kingdom::get_citizensinkingdom_count( $region -> kingdom_id );
			...
			$structures = Model_Region::get_structures_d( $region -> region_id );
		...
		******************************************/
		...
		// Finch� la seconda regione appartiene al regno delle prima, scorro l'array
		$second_region = array();
		...
		foreach ($apocalypse as $regiontoattack)
		{
			...
			// Se � presente invio un evento di notifica al personaggio
			if ( is_null( $king ) )
			{
