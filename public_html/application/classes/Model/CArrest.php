...
class Model_CharacterAction_CArrest extends Model_CharacterAction
{
	...
	{ 
		...
		// se c'� n'� almeno uno che fa riferimento ad una procedura aperta, il controllo
		// � passato.
		...
		///////////////////////////////////////////////////////////////////////		
		// non si può arrestare un reggente o un leader religioso
		...
		//////////////////////////////////////////////////////////////////////
		// Il regno del giocatore � in guerra con il regno di chi tenta di
		// arrestarlo?
		//////////////////////////////////////////////////////////////////////
		
		$guardcaptainkingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[0] -> region -> kingdom_id, 'running');
		$criminalkingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[1] -> region -> kingdom_id, 'running');
		...
		//////////////////////////////////////////////////////////////////////
		// Se il regno � in rivolta non si può arrestare
		//////////////////////////////////////////////////////////////////////		
		
		$runningbattles = Model_Kingdom::get_runningbattles( $par[1] -> region -> kingdom_id );
	...
	public function append_action ( $par, &$message ) 
	{
		...
		$a =  Model_CharacterAction::factory("arrest");
		$a -> starttime = $starttime;
		$a -> endtime = $endtime;
		$a -> status = 'running';
		$a -> param1 = $this -> currentregion -> id ;
		$a -> param2 = $court -> region -> id ;
		// parametro per differenziare chi � arrestato
		...
		// azione bloccante per chi � arrestato
		$a = Model_CharacterAction::factory("arrest");
		$a -> starttime = $starttime;
		$a -> endtime = $endtime;
		$a -> status = 'running';
	...
	public function complete_action( $data)
	{
		...
		$distance = Model_RegionPath::compute_distance( $sourceregion -> name, $targetregion -> name );
		...
		// se param3 � true, appendo un ordine di restrizione per 2 giorni
	...
	{		
		...
		$distance = max( Model_RegionPath::compute_distance(
