...
class Model_Item_ITScroll extends Model_Item
{
/**
...
function ec_scroll_generic( $item )
{
	...
	$content['scroll_body'] = Model_Utility::bbcode($item->param3);
	$content['scroll_signature'] = Model_Utility::bbcode($item->param2);
	return $content; 
}
...
public function buy_do_proprietary_check( $boughtquantity, &$message )
{
	...
	if ( $this -> cfgitem -> tag == 'scroll_propertylicense' )
	{
		...
		// il compratore possiede gi� un negozio?
		if ( $structure -> structure_type -> parenttype == 'shop' ) 
		{
			...
			else if ($ownedshops == Kohana::config('medeur.maxshops'))		
			{
				...
				$churchhasoraetlaborabonus = Model_Church::has_dogma_bonus( $character -> church_id, 'craftblessing');
...
{	
	...
	$structure = Model_StructureFactory::create( null, $procedure -> structure_id );
