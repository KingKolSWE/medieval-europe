...
class Model_CharacterAction_CARevokerolerp extends Model_CharacterAction
{
	...
	public function execute_action ( $par, &$message) 
	{
		...
		Model_Item::consume_equipment( $this->equipment, $par[1] -> character );
		...
		Model_CharacterEvent::addrecord( $par[0]->character->id, 'normal', $msg2 );
		
		// rimuovo eventuali grant
		if ( $par[0] -> tag == 'chancellor' )
		{
			$royalpalace = $par[1] -> region -> get_controllingroyalpalace();
			$ca = new Model_CharacterAction_CARevokestructuregrant();
			$_par[0] = $royalpalace;
			$_par[1] = $revoked;
			$_par[2] = 'chancellor';
