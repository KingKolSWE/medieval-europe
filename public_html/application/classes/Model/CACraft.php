...
class Model_CharacterAction_CACraft extends Model_CharacterAction
{
	...
	*/
	...
	public function cancel_action() 
	{
		...
		$structure = Model_StructureFactory::create( null, $this -> param2 );
		
		if ( Model_StructureGrant::get_chargrant( $structure, $character, 'worker' ) == true )
			return false;

		Model_StructureEvent::newadd( $this -> param2,
	...
	protected function append_action( $par, &$message )
	{
		...
		foreach ( $neededitems as $item)
		{
			if ( $item -> type == 'item' )
			{
				$i = Model_Item::factory( null, $item -> tag );
		...
		{			
			if ( $item -> type == 'tool' )
				Model_Item::consumeitem_instructure(
					$item -> tag, 
	...
	{		
		...
		$i = Model_Item::factory( null, $cfgitem->tag);
		$char = ORM::factory( "character", $data->character_id );
		$structure = Model_StructureFactory::create( null, $data -> param2 );
		...
		Model_GameEvent::process_event( $char, 'craft', $par );
		...
		if ( $char -> id != $structure -> character_id )
			Model_Job::givehourlywage( $structure, $char, round( ($data -> param4 / 60) * $data -> param3 ) );
