...
class Model_CharacterAction_CADeclarewaraction extends Model_CharacterAction
{
	...
	protected function check( $par, &$message )
	{
		...
		$attackingkingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[1] -> kingdom_id, 'running');
		...
		$defendingkingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[3] -> kingdom_id, 'running');
		...
		$diplomacyrelation = Model_DiplomacyRelation::get_diplomacy_relation( $par[1] -> kingdom_id, $par[3] -> kingdom_id);
		...
		if ( $par[2] == 'conquer_r' and $par[3] -> capital and Model_CharacterRole::check_eligibility( $par[4], 'king', null, $message ) == false  )
		...
		$data = null;
		$iskingdomfighting = Model_Kingdom::is_fighting( $par[1] -> kingdom_id, $data ) ;
		...
		$iskingdomfighting = Model_Kingdom::is_fighting( $par[3] -> kingdom_id, $data );
		...
		$diplomacyrelationwithkingdomthatdeclaredwar = Model_DiplomacyRelation::get_diplomacy_relation(
