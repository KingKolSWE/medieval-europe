...
class Model_Course
{
	...
	*/
	...
	public function helper_show( $char, $structure )
	{
		...
		if ( $this -> getCoursetype() == 'skill' )
		{
			...
			if ( Model_Skill::character_has_skill( $char -> id, $this->getLinkedskill()) )
			{
				$html = '<fieldset>';
				$html .= '<legend>' . $this -> getName() . '</legend>' ;		
				$html .=  "<p class='center'><i>" . $this -> getDescription() . '</i></p>' ;
	...
	*/
	...
	{		 
		...
		{			
			...
			$skills = Model_Skill::get_character_skillcount( $char -> id );
	...
	*/
	...
	public function getPricePerHour( $char, $structure )
	{
		...
		$valueaddedtax = Model_Region::get_appliable_tax( $structure -> region, 'valueaddedtax', $char );
