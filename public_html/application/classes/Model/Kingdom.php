...
class Model_Kingdom extends ORM
{
	...
	*/
	...
	{
		$allies = array();
		$diplomacyrelations = Model_DiplomacyRelation::get_diplomacy_relations( $kingdom_id );
	...
	*/
	...
	function get_last_war( $kingdom_id )
	{
		...
		$lastwar = null;
		$kingdomwars = Model_Kingdom::get_kingdomwars( $kingdom_id );
	...
	*/
	...
	function are_kingdoms_at_war( $kingdom1_id, $kingdom2_id )
	{
		...
		$wars = Model_Kingdom::get_kingdomwars( $kingdom1_id, 'running' );
	...
	/**
	* Guerre in cui il regno � impegnato, sia come main attacker sia come alleato
	* @param int $kingdom_id ID Regno
	* @param str $status 'all' o 'running' 
	* @return array $kingdomwars Tutte le guerre in cui il regno � coinvolto
	*/
	
	function get_kingdomwars( $kingdom_id, $status = 'all' )
	{
		$allwars = Model_Configuration::get_kingdomswars();
	...
	*/
	...
	public static function get_position_for_stat( $kingdom_id, $statname )
	{
		...
		$cfgkingdoms = Model_Configuration::getcfg_kingdoms();
	...
	*/	
	...
	function dethrone_king( )
	{
		...
		// se non c'� il Re, non c'� niente da fare.
		if ( ! $king -> loaded )
			return;
	...
	*/	
	...
	function crown_king( $char )
	{
		...
		Model_CharacterRole::start( $char, 'king', $royalpalace -> region, $royalpalace -> id, null, false );
	...
	*/
	...
	public static function  get_destination_region( $choosenkingdom_id )
	{
		...
		$capital = Model_Kingdom::get_capitalregion( $choosenkingdom_id );
	...
	*/
	...
	function get_assignableregions()
	{
		...
		{					
			...
			// se il castello � presente la regione non � assegnabile.
			if ( !is_null( $castle ) )
	...
	*/
	...
	function get_info()
	{
      ...
      and   c.type != 'npc' 			
		...
		// stats_globals
		$info['richestkingdomposition'] = is_null(Model_Kingdom::get_position_for_stat( $this -> id, 'richestkingdoms')) ? kohana::lang('global.notclassified') : Model_Kingdom::get_position_for_stat( $this -> id, 'richestkingdoms') ;
		$info['populatedkingdomposition'] = is_null(Model_Kingdom::get_position_for_stat( $this -> id, 'populatedkingdoms')) ? kohana::lang('global.notclassified') : Model_Kingdom::get_position_for_stat( $this -> id, 'populatedkingdoms') ;
		$info['activekingdomposition'] = is_null(Model_Kingdom::get_position_for_stat( $this -> id, 'activekingdoms')) ? kohana::lang('global.notclassified') : Model_Kingdom::get_position_for_stat( $this -> id, 'activekingdoms') ;
		$cfgkingdoms = Model_Configuration::getcfg_kingdoms();
