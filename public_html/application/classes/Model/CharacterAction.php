...
class Model_CharacterAction extends ORM
{
	...
	/**
	* Funzione che controlla se � possibile effettuare un azione
	...
	*/
	...
	protected function check_( $par, &$message, $callerchar_id = null, $targetchar_id = null )
	{
		...
		// controllo se l' azione � applicabile ad un npc
		...
		{
			// Verifico se l'equipaggiamento indossato � corretto
	...
	*/
	...
	public function do_action( $par, &$message )
	{
		...
		// se get_timetocomplete � = 0 semplicemente chiamo execute_action
		...
		try
		{
			...
			else
			{
				...
				// aggiorno la cache, solo se la action � non immediata		
				kohana::log( 'debug', "-> Deleting currentpendingaction from cache for char {$this -> character_id}");
				Model_MyCache::delete(  '-charinfo_' . $this -> character_id . '_currentpendingaction');
			}
	...
	* La tabella character_actions � stata modificata
	...
	*/
	...
	{	
		...
		// Verifico se c'� un azione running nel db scaduta
		...
		foreach ( $result as $row )
		{
			try 
			{
				...
				// Se l'azione non � ciclica allora la setto come completata
				
				if ( $row -> cycle_flag == FALSE )
				{ 
					$db -> query( "update character_actions set status = 'completed' where id = " . $row -> id ); 
					Model_MyCache::delete(  '-charinfo_' . $row -> character_id . '_currentpendingaction' );
					
					// metto il char a dormire se l' azione � blocking, ed evito il loop.
				...
				// Se l'azione � ciclica allora blanko la chiave per la prossima complete		
	...
	*/
	...
	{					
		...
		if ( $o -> loaded )
		{
			if ( Model_CharacterAction::factory( $o -> action ) -> immediate_action == true )
				return null;
			else
				return $o;
		}
	...
	* @param  force_flag se true, l' azione viene forzata anche se non � stata ordinata
	*         dal giocatore
	...
	*/
	...
	public function cancel_pending_action( $character_id = null, $force_flag = false, &$message = '')
	{
		...
		{			
			...
			$action = Model_CharacterAction::factory( $pendingaction['action'] ) -> find( $pendingaction['id'] );
			...
			kohana::log('debug', "-> Result from action cancel action: [{$rc}], message: [{$message}]" );
				...
				if ( $rc ) 
				{
					...
					Model_MyCache::delete(  '-charinfo_' . Session::instance()->get('char_id') . '_currentpendingaction' );
					return true;
				}
				else
					return false;
	...
	*/
	...
	public function get_action_time( $character )
	{
		...
		kohana::log('debug', '-> Original time: ' . Model_Utility::secs2hmstostring( $basetime ) );
		...
		kohana::log('debug', '-> Original time after serverspeed factor: ' . 		Model_Utility::secs2hmstostring($basetime ));
		...
		(
			...
			Model_Church::has_dogma_bonus($character -> church_id, 'resourceextractionblessing') )
		{
		...
		(
			...
			Model_Church::has_dogma_bonus($character -> church_id, 'craftblessing'))
		{
		...
		(
			...
			Model_Church::has_dogma_bonus($character -> church_id, 'concentrateandlearn'))
		{
		...
		// Debug
		kohana::log('debug', '-> Final time: ' . Model_Utility::secs2hmstostring( $modifiedtime ) );
	...
	* Inserendo la key 'all' � possibile specificare gli items che devono
	* indossare tutti i chars che non hanno un ruolo
	* Il tag '*' significa che va bene qualsiasi tipo di item/vestito
	* Il check viene superato se nella zona del corpo � presente almeno uno
	...
	********************************************************************/
	...
	public function obs_check_equipment ($equipment, $char, &$equipmentincorrect)
	{
		...
		// Check: equipment_to_check non � nulla
		...
		if ( ! is_null($equipment_to_check) )
		{
			...
			// allora devo eseguire dei controlli dedicati perch�
			// � sufficiente che venga soddisfatta la presenza di items
			...
			{	
				...
				// Se l'item indossato � nullo ed � previsto che il char
				// indossi qualsiasi cosa oppure c'� un array definito di
				...
				// Il controllo � superato solo se:
				// 1) il check sul body � negativo ma torso e legs vanno bene 
				...
				// body, torso e legs � superato e quindi li rimuovo dall'array
			...
			// per ogni parte del corpo. L'array viene prelevato nella modalità 
			// Key (parte del corpo) => Value (array di items che può indossare)
			...
			foreach ($equipment_to_check as $bodypart => $array_of_items)
			{
				...
				// Check: sono specificati uno o più items da indossare per eseguire l'azione
