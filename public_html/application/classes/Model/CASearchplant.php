...
class Model_CharacterAction_CASearchplant extends Model_CharacterAction
{
	// Costanti	
	const DELTA_GLUT = 5;
	...
	{ 
		...
		$rd = Model_DiplomacyRelation::get_diplomacy_relation(
		...
		// controlliamo, se il parametro coda � > 1 quanti item estrarrebbe il char. Se ce ne sono di meno nella 
		// risorsa -> errore
	...
	protected function append_action( $par, &$message )
	{
		...
		// toglie la quantit� alle risorse e appende l' azione.
	...
	/*
	* Prevede quante unit� estrarre in base alle caratteristiche
	* del char e ad altri parametri
	* @param $par parametri passati 
	* @return vettore item e quantit� da estrarre
	*/
	
	private function computeproduction( $par )
	{		
		
		$item = Model_Item::factory( null, 'medmushroom' );
		$items[ $item -> cfgitem -> tag ] = $item -> computeproduction( $par[1] ) * $par[2];
		
		$item = Model_Item::factory( null, 'mandragora' );
		$items[ $item -> cfgitem -> tag ] = $item -> computeproduction( $par[1] ) * $par[2];		
	...
	public function complete_action( $data )
	{
		...
		foreach ( $items as $item )
		{
			...
			else
			{
				...
				Model_CharacterEvent::addrecord( $char -> id, 'normal', '__events.searchitemnotok', 'normal' );
			}
		}				
