...
class Model_CharacterAction_CAEntercity extends Model_CharacterAction
{
	...
	*/
	...
	{ 
		...
		// E' possibile entrare in città solo se le azioni diplomatiche lo consentono.		
		// check se il rapporto tra il regno e il regno del char � ostile
		...
		// Se il kingdom B � in guerra, il char non può muoversi se
		// � alleato di almeno un regno che sta attaccando
		
		$data = null;
		$isonwar = $par[1] -> region -> kingdom -> is_fighting( $par[1] -> region -> kingdom_id, $data);		
		
		if ( $isonwar !== false )		{
			// per ogni battaglia, verifichiamo se il char � alleato di un regno
	...
	public function execute_action ( $par, &$message ) 
	{
		...
		Model_CharacterEvent::addrecord( $par[0] -> id , 'normal', '__events.battlefield_leave');
