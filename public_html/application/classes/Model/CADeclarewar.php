...
class Model_CharacterAction_CADeclarewar extends Model_CharacterAction
{
	...
	{ 					
		...
		$dr = Model_DiplomacyRelation::get_diplomacy_relation( $par[0] -> region -> kingdom_id, $par[1] -> id );
		...
		$attackingkingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[0] -> region -> kingdom_id, 'running');
		...
		$defendingkingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[1] -> id, 'running');
		...
		$lastwar = Model_Kingdom::get_last_war( $par[0] -> region -> kingdom_id );
	...
	public function execute_action ( $par, &$message) 
	{
		...
		$sourcekingdomallies = Model_Kingdom::get_allies($par[0] -> region -> kingdom_id);
		$targetkingdomallies = Model_Kingdom::get_allies($par[1] -> id);

		// Agg. Kingdom That declares
		$kingdomwar_kingdom = new Model_KingdomWarsAlly();
		...
		Model_MyCache::delete('-cfg-kingdomswars');
