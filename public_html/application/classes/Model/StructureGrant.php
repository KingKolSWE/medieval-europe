...
class Model_StructureGrant extends ORM
{
...
* @param granted oggetto char a cui � dato il permesso	
* @param job oggetto job a cui � legato il grant
* @return none
*/	
...
function get_chargrants( $structure, $granted )
{
	...
	$cachetag = '-charstructuregrant_' . $granted -> id . '_' . $structure -> id;
	$grants = Model_MyCache::get( $cachetag );
	...
	if ( is_null( $grants ) )		
	{
		...
		Model_MyCache::set( $cachetag, $grants, (24*3600) );
...
function get_chargrant( $structure, $granted, $grant )
{
	...
	$grants = Model_StructureGrant::get_chargrants( $structure, $granted );
