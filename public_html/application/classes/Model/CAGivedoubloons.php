...
class Model_CharacterAction_CAGivedoubloons extends Model_CharacterAction
{
	...
	public function execute_action ( $par, &$message) 
	{
		...
		// se la causale � purchase aumenta la stat
		// e dai la quota al referrer solo se il char che INVIA non � un ADR.
		...
		{	
			...
			$referrer = Model_UserReferral::get_referrer( $par[0] );
			$quantity = max(1, round($par[1]*5/100,0));
			
			if ( !is_null( $referrer ) )							
			{
				
				kohana::log( 'debug', $referrer -> name . ' is referrer?' . Model_User::has_role( $referrer -> user, 'doubloonreseller' ));
				// Se il Referrer � un ADR, non dare dobloni.				
				
				if ( Model_User::has_role( $referrer -> user, 'doubloonreseller' ) == true )
				{
