...
class Model_KingdomForumBoard extends Model_Board
{			
	...
	function read( $char, $board_id, &$board )
	{
		...
		if ( Model_KingdomForumBoard::haswriterights( $char, $board -> kingdom ) == false )
		{ 
			$message = 'global.operation_not_allowed';
			return false;
		}
	...
	function add_model($char, $data, &$message )
	{
		...
		$kingdom = Model_Kingdom::load( $data['kingdom_id'] );
		...
		if ( Model_KingdomForumBoard::haswriterights( $char, $kingdom ) == false )
		{ 
			$message = 'global.operation_not_allowed';
			return false;
		}
