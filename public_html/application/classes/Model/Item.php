...
class Model_Item extends ORM
{
	...
	* @param string $page pagina in cui � chiamato il tooltip
	...
	*/
	...
	{		
		...
		$title .= '<tr><td style="left" width="100px">' . kohana::lang('items.condition'). '</td><td>' . Model_Utility::number_format($item -> quality,2) . '%</td></tr>';
		...
		if ( $item -> tag == 'scroll_propertylicense' )
		{
			...
			// se lo scroll di propriet� punta ad una struttura non esistente, cancella l' item.
	...
	*/	
	...
	function get_actions ( $item )
	{
		...
		// mostra take solo se l' item � sul terreno.
		...
		// Se l' item � di classe container, aggiungo open
		...
		// Se l'item � consumabile aggiunge apply
		...
		// Se l'item � un armor, una weapon, un clothes o un tool allora visualizzo l'azione 'indossa'
		...
		// Se l'item � un armor o una weapon ed � prestato, aggiungo 'ritorna'
		if ( !is_null( $item -> lend_id ) )		
		{ $actions[] = 'returnlentitem'; }
		
		// Se l'item � un vestito colorabile allora visualizzo l'azione 'colora'
		...
		// Se l' item � un armor o un weapon ed � equipaggiato visualizzo l' azione undress
		if (
			in_array($item->parentcategory, array(
				'tools',
				'armors',
		...
		// se l' item � scroll aggiungo leggi
		if ( $item->category=='scroll' )
		{ $actions[] = 'read'; }

		// se l' item � uno scroll generico aggiungo esibisci
		...
		// se l' item � un pezzo di carta aggiungo scrivi
		if ( $item->tag=='paper_piece' )
		{ $actions[] = 'write'; }
		
		// se l' item � un cart pro, aggiungo REST
		if ( $item -> tag == 'cart_3' )
		{ $actions[] = 'rest'; }
	...
	*/
	...
	public function additem( $destination, $id, $quantity = 1, $equipped = null)
	{
		...
		// se la quantit� � <= 0, non fare niente poich� siamo nella additem.
		
		if ( $quantity <= 0 ) 
			return;
				
		/////////////////////////////////////////////////////////////////////
		// se il tipo di item � scroll, l' item � locked, o la lend_id � 
		// popolata, non � raggruppabile per definizione e devo modificare 
		...
		else
		{
			...
			{
					$structure = Model_StructureFactory::create( null, $id );
					
					// se si aggiunge un item al mercato, deve essere 
					// raggruppato per seller id, prezzo e qualit� ecc.
		...
		/////////////////////////////////////////////////////////////////////////////////
		// Se � groupable, ed � stato trovato un gruppo di oggetti simili, aggiungo 
		// l' item al gruppo.		
		/////////////////////////////////////////////////////////////////////////////////
		...
		{					
			...
			if ( $destination == 'character' )		
			{				
				Model_MyCache::delete(  '-charinfo_' . $id . '_' . $this -> cfgitem -> tag );
			}
		}
		
		/////////////////////////////////////////////////////////////////////////////////
		// Se non � groupable, o � groupable MA non � stato trovato un gruppo di oggetti 
	...
	/** 
	* Rimuove un item
	* @param destination: "character" oppure "structure": indica a che entit� deve essere rimosso l' item
	* @param id: id della entit�
	* @param quantity: quantit� da aggiungere
	* @return: esito
	*/
	...
	public function removeitem( $destination, $id, $quantitytoremove = 1)
	{
		...
		// se l' oggetto non � groupable, operiamo sull' item stesso.
		...
		else
		{
			...
			// se l' oggetto � groupable, troviamo se esiste un gruppo di item simili
	...
	/** 
	* Funzione per consumare la qualit� di un item
	...
	*/
	...
	public function consumeitem_instructure( $tag, $structure_id, $amounttoconsume )		
	{
		...
		foreach ( $items as $item )
		{
			...
			// se � stato toccata la auqlit� e l' item ha quantity 1, salva la nuova qualit�
			if (
				$item -> quality > 0 
				and 
			...
			// se � stato toccata la auqlit� e l' item ha quantity > 1, scorpora
			if ( 
				$item -> quality > 0 
				and 
				$item -> quality != $item -> quality_copy 
				...
				)
			{
					...
					// separiamo un item identico settandogli la qualit�.
	...
	public function obs_consumeitem_instructure( $tag, $structure_id, $amount )	
	{
		...
		// stabiliamo quanti item e qualit� ci sono in totale.
		...
		// Cerchiamo se esiste un item scorporato e con qualit� < 100. 
		// se c'�, consumiamo quello.
		...
		while ( $lefttoconsume > 0 )
		{
			...
			// priorit� agli item scorporati e con qualit� < 100
			...
			// se l' item c'� ma � raggruppato, lo scorporo.		
	...
	* @return valore da consumare alla qualit� dell'item 
	*/   
	...
	* @param int $quantity: quantit� da inviare
	...
	*/
	...
	function computesenddata ( $quantity, $item_id, $source, $targetname, $action )
	{
		...
		// Fai controlli di consistenza solo se l' action � send.
		...
		// Stabiliamo il prezzo su quantit� oggetti inviati
		...
		// Se � una send e il char sta inviando soldi, il char deve avere i soldi che sta inviando
		// pi� il costo di invio.
		...
		$info['cost'] = Model_Utility::number_format($finalcost,2);
		
		// tempo per inviare l' oggetto
		
		$distance = Model_RegionPath::compute_distance( $source_region -> name, $target_region -> name );
		...
		$info['timetext'] = Model_Utility::format_datetime($info['time']);
	...
	* @param $mode: se normal ci si aspetta che l' oggetto item 
	         sia un istanza della classe item altrimenti se � 'light' 
	         l' oggetto item � un vettore
	* @return prezzo		 
	*/
	...
	public function compute_realprice( $item, $char, $vat, $mode = 'normal' )
	{
		...
		if ( $mode == 'normal' )		
			$flatitem = Model_Item::get_flatitemdata( $item -> id );
		else
			$flatitem = $item;
		...
		// In caso di dobloni, la tassa non � applicata fino
	...
	* ['bodypart'] = value
	* Per ogni ruolo � possibile specificare un array di zone del
	...
	********************************************************************/
	...
	public function consume_equipment ($equipment, $char, $multiplier = 1)
	{
		...
		// Check: il ruolo � presente nella lista di quelli da controllare
		
		if ( !is_null($role) and array_key_exists($role->tag, $equipment) )
		{
			$equipment_to_consume = $equipment[$role->tag];
		}
		// Il char non ha un ruolo ma � definito un array
		...
		// Seleziono la parte del corpo. L'array viene prelevato nella modalit� 
		...
		$category = 'all' )	
	{
		...
		else
		{
			...
			{	
				...
				// se il recipient_id non � uguale al char id
				...
				$title = Model_Item::helper_tooltip( $item, 'market' );
				...
				$html .= "<td class='center $class'>" . Model_Utility::number_format($item -> quality, 2) . "%</td>";
				...
				if ( $item -> seller_id != $character -> id  )
				{
					...
					// Lo sceriffo pu� confiscare oggetti		
		...
		else
		{
			...
			{	
				...
				{	
					...
					$title = Model_Item::helper_tooltip( $item, 'structureinventory' );
					...
					$html .= "<td class='right'>" . Model_Utility::number_format($item -> quality, 2) . "%</td>";
					...
					$html .= "<td class='right'>" . form::input( 
						array( 
							...
							'class' => 'input-xxsmall right' ) )
						. "</td>"; 
					$html .= "<td class='right'>" . Model_Utility::number_format( $item -> totalweight/1000,1) . "  </td>";
					if ($mode == 'character' )					
						$html .= "<td class='right'>" . 
							form::submit( 
							array 
	...
	$category = 'consumables' )
	{
		...
		else
		{
			...
			{		
				...
				$title = Model_Item::helper_tooltip( $item, 'inventory' );
				...
				// Se l'oggetto � uno scroll generico allora visualizzo il titolo al posto del
				// nome dell'oggetto
				...
				if ($item -> quality < 25)
					$class = 'poorqualityitem';				
				$html .= "<td style='text-align:right' class='$class'>". Model_Utility::number_format($item -> quality, 2) . "%</td>";
		...
		else
		{
			...
			foreach ( $items['items'][$category] as $items ) 				
			{
				...
				{	
					...
					$class = ($k % 2 == 0) ? 'alternaterow_1' : '';
					$title = Model_Item::helper_tooltip( $items, 'market' );
