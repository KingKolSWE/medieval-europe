...
class Model_CharacterAction_CACurehealth extends Model_CharacterAction
{
	...
	{ 
		...
		$has_dogma_bonus = Model_Church::has_dogma_bonus($par[0] -> church_id, 'curehealthextended');
	...
	*/
	...
	protected function append_action( $par, &$message )
	{
		...
		$has_dogma_bonus = Model_Church::has_dogma_bonus($this -> controlledstructure->structure_type->church_id,'curehealthextended');
		...
		if ( Model_Character::is_recovering( $par[1] -> id) == true )
		{
			...
			$rc = Model_CharacterAction::cancel_pending_action( $par[1] -> id, true, $message );
			if ($rc == false )
				return $rc;
		}
	...
	public function complete_action( $data )
	{
		...
		********************************/
		...
		{	
			...
			$has_dogma_bonus = Model_Church::has_dogma_bonus($structure->structure_type->church_id,'curehealthextended');
			
			// Restore health
			
			$hptorestore = Model_CharacterAction_CACurehealth::get_hptorestore( $chartarget, $has_dogma_bonus);
	...
	*/
	...
	public function get_action_time( $sourcecharacter, $targetcharacter )
	{
		...
		kohana::log('debug', '-> Time now: '. Model_Utility::secs2hmstostring($time));
