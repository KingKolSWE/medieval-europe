...
class Model_CharacterAction_CAChangecity extends Model_CharacterAction
{
	...
	{ 
		...
		// controllo se c'� la diplomazia ostile tra i regni
		...
		// Non � possibile trasferirsi in una regione piena
		if ( $par[1] -> is_full() )		
	...
	public function execute_action ( $par, &$message) 
	{
		...
		// memorizzo quando ha cambiato la citt�, ma anche quando ha cambiato il regno.
		...
		Model_CharacterEvent::addrecord( $par[0]->id, 'normal', '__events.city_change_eventtext;__'.$par[1]->name);
		...
		if ( !is_null ( $king ) )
		{
			...
			Model_CharacterEvent::addrecord( $king->character_id, 'normal', '__events.city_change_info_source' .
		...
		if ( !is_null( $vassal) ) 
		{
			...
			Model_CharacterEvent::addrecord( $vassal->character_id, 'normal', '__events.city_change_info_source' .
				';' . $par[0]->name . ';__' . $source->kingdom -> get_name()   .  ';__'.$source->name  .
		     ';__' . $dest->kingdom -> get_name()   . ';__' . $dest->name,
				'evidence' 
				);
		...
		if ( !is_null ( $king ) )
		{
			...
			Model_CharacterEvent::addrecord( $king->character_id, 'normal', '__events.city_change_info_dest' .
		...
		if ( !is_null( $vassal) ) 
		{
			...
			Model_CharacterEvent::addrecord( $vassal->character_id, 'normal', '__events.city_change_info_dest' .
				';__'.$dest->kingdom -> get_name()   .  ';__' . $dest->name .  ';' . $par[0]->name . 
				';__' . $source->kingdom -> get_name()  . ';__' . $source->name,
				'evidence' 
				);
