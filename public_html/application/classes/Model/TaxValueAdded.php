...
class Model_TaxValueAdded
{

	/*
	* Applica la tassa
	* @param par vettore di parametri
	*                par[0] = struttura dove si � acquistato il bene o servizio
	...
	*/
	...
	public function apply ( $par ) 
	{
		...
		$distributiontax = Model_Kingdom::get_tax( 'distributiontax', $par[0] -> region -> kingdom_id );
		...
		if ( $castle_coins > 0 )
		{
			...
			'__taxes.income_' . $par[5] . ';' . 
			'__' . $par[0] -> region -> name ;
			Model_StructureEvent::newadd( $castle -> id, $text );
		}
