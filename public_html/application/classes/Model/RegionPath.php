...
class Model_RegionPath extends ORM
{
	...
	*/
	...
	function get_travelinfo( $par )
	{
		...
		kohana::log('info', '-> Original time: ' . $travelinfo['normaltraveltime'] . ' minutes - ' . Model_Utility::secs2hmstostring($travelinfo['normaltraveltime'] * 60, 'hours' ));
		kohana::log('info', '-> Excess weight: ' . $weightinexcess );
		kohana::log('info', '-> Time after weight penalty: ' . $travelinfo['realtraveltime'] .' - '. Model_Utility::secs2hmstostring($travelinfo['realtraveltime'] * 60, 'hours'));
		...
		if ( ($par['type'] == "land" || $par['type'] == "fastland" ) and is_null($par['hasshoes']) )
		{
			...
			kohana::log('info', '-> Time after shoes malus penalty: ' . $travelinfo['realtraveltime'] .' - '. Model_Utility::secs2hmstostring($travelinfo['realtraveltime']* 60, 'hours') );
		...
		kohana::log('info', '-> Time after glut/energy malus: ' . $travelinfo['realtraveltime'] .' - '. Model_Utility::secs2hmstostring($travelinfo['realtraveltime']* 60, 'hours') );
		...
		kohana::log('debug', '-> Time after Cost check: ' . $travelinfo['realtraveltime'] . ' - '. Model_Utility::secs2hmstostring($travelinfo['realtraveltime']* 60, 'hours') );
		...
		kohana::log('info', '-> Real Travel time after traveler package check: ' . Model_Utility::secs2hmstostring($travelinfo['realtraveltime'] * 60, 'hours'));

		// server speed

		$travelinfo['realtraveltime'] = max (1, $travelinfo['realtraveltime']/Kohana::config('medeur.serverspeed'));

		kohana::log('info', '-> Real Travel time after applying bonus speed: ' . Model_Utility::secs2hmstostring($travelinfo['realtraveltime'] * 60, 'hours'));

		$travelinfo['realtraveltimetext'] = kohana::lang('global.traveltime') . "<span class='value'>" .
			Model_Utility::secs2hmstostring($travelinfo['realtraveltime'] * 60, 'hours') . "</span>";

		$travelinfo['costtext'] = kohana::lang('global.price') . ": <span class='value'>" . $travelinfo['cost'] . "</span>";

		kohana::log('info', '-> Time after horse or noble bonuses: ' . $travelinfo['realtraveltime'] .'-'. Model_Utility::secs2hmstostring($travelinfo['realtraveltime']* 60) );
	...
	*/
	...
	{
		$cfg_regions = Model_Configuration::get_cfg_regions();
	...
	*/
	...
	function ispathcrossable( $char, $currentregion, $fasttrackpath )
	{
		...
		foreach ($fasttrackpath['crossedregions'] as $crossedregion )
		{
			...
			$possibletocross = Model_Region::canmoveto( $char, $crossedregionobj, $currentregion, false, $message );
