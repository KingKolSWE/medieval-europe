...
{	
	...
	* @param boolean flag che determina se il ruolo � GDR
	*/
	
	function start( $char, $roletag, $region, $structure_id, $place, $is_gdr )
	{	
		
		$cr = new Model_CharacterRole();
		...
		if ( ! $is_gdr )
		{
			...
			// a lui spettanti.
			// Esempio: il vassallo dovr� controllare, in 
			// tutte le regioni che controlla:
			// castello, e progetti a lui assegnati
			// Esempio: un cardinale controller� solo la
			// regione dove c'� il  pal. cardinalizio		
			//////////////////////////////////////////////
	...
	*/
	...
	/** 
	* verifica se il char pu� assumere il ruolo
	...
	function check_eligibility( $char, $role, $church, &$message )
	{
		...
		*/
		...
		if ( in_array( $role, array('church_level_2', 'church_level_3', 'church_level_4' ) ) )
		{
			...
			{						
				if ( Model_CharacterRole::charhasservedrole( $char, 'church_level_3' ) == false )
				{
			...
			{						
				if ( Model_CharacterRole::charhasservedrole( $char, 'church_level_4' ) == false )
				{
	...
	*/
	...
	function get_controlledstructures()
	{
		...
		// cariche ecclesiastiche, la regione � quella legata al ruolo
		
		else if ( in_array( $this -> tag, 
			array ('church_level_1', 'church_level_2', 'church_level_3', 'church_level_4' ) ) )
			$crs[] = $this -> region ;	
		
		// altro, la regione � quella la regione � quella legata al ruolo
	...
	* Se � un ruolo religioso, il controllo si estende
	* al parametro chiesa.
	* @param char: oggetto char
	...
	*/
	...
	/**
	* Ritorna se un ruolo � di governo o di chiesa
	* @param none
