...
class Model_CharacterAction_CARepair extends Model_CharacterAction
{
	...
	{ 
		...
		// il char � nella stessa regione della struttura?		
	...
	protected function append_action( $par, &$message )	{	
		...
		$item = Model_Item::factory(null, 'wood_piece');
		$item -> removeitem( "structure", $par[0] -> id, 1 * $par[2]);
		$item = Model_Item::factory(null, 'iron_piece');
		$item -> removeitem( "structure", $par[0] -> id, 1 * $par[2]);
	...
	public function complete_action( $data )	{	
		...
		// incrementa la qualit� della struttura
		// cap a 100%
		if ( $structure -> loaded )
		{
		...
		// incremente il livello di fede solo se il char � della stessa religione
		if ( $structure -> structure_type -> church_id == $character -> church_id )
			$character -> modify_faithlevel( $data -> param2 * 2 );
		
		// aumenta il numero di AFP solo se il char � della stessa religione
		if ( $structure -> structure_type -> church_id == $character -> church_id )
			$character -> modify_stat( 'fpcontribution', $data -> param2 * 5, $character -> church_id );			
				
		// diminuisce il livello di fede solo se il char � di diversa religione	
		...
		// decremente il numero di AFP per ogni danneggiamento se il char � di diversa religione
		if ( $structure -> structure_type -> church_id != $character -> church_id )
	...
	public function get_action_message( $type = 'long') 
	{
		...
		$message = "";				
		$structure = Model_StructureFactory::create( null, $pending_action -> param1 );
