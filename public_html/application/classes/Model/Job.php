...
class Model_Job extends ORM
{
	...
	*/
	...
	function delete( $char, $job, &$message )
	{
		...
		// chi cancella pu� essere solo l' employer
		...
		if ( $structure -> loaded )
			Model_StructureGrant::revoke( $structure, $job -> character, $job, 'worker'  );
	...
	*/
	...
	function republish( $char, $job, &$message )
	{
		...
		// chi ripubblica pu� essere solo l' employer
		
		if ( $char -> id != $job -> employer_id )
		{	$message = kohana::lang('global.operation_not_allowed'); return false; }
	
		// si pu� ripubblicare un annuncio solo se non � scaduto
	...
	*/
	...
	function accept( $employee, $employer, $announce, $structure, &$message )
	{
		...
		// chi accetta pu� essere solo l' employee
		...
		if ( !is_null( $structure ) )
		{
			// c'� gi� un contratto attivo per lo stesso char sulla struttura?
			...
			// c'� gi� un contratto attivo sulla struttura e nello shop non c'� lo sturdyworkbench?
			...
			if ( $structure -> structure_type -> parenttype == 'shop' ) 
			{
				...
				// ci sono gi� attivi 5 contratti sulla struttura e nello shop non c'� lo sturdyworkbench?
		...
		// d� una grant sulla struttura
		if ( !is_null( $structure ) )
			Model_StructureGrant::add_model(
				$structure, 
				$employee, 
				$job, 
				'worker', 
	...
	/**
	* Paga oraria
	* @param structure oggetto struttura dove il lavoro � fatto
	...
	*/
	...
	public function givehourlywage( $structure, $employee, $workedhours )
	{
		...
		if ( Model_StructureGrant::get_chargrant( $structure, $employee, 'worker' ) == true )
		{
