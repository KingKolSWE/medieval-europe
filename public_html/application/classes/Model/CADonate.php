...
class Model_CharacterAction_CADonate extends Model_CharacterAction
{
	...
	{ 
		...
		// il giocatore � nella stessa regione della struttura?		
		...
		// si pu� donare/trashare l' oggetto?
		...
		// check: il peso degli item supera la capacit� di immagazzinamento della struttura?
		$itemsweight = $par[1] -> get_totalweight( $par[2] ); 
		$storableweight = $par[0] -> get_storableweight( $par[0] );
		
		// check: si pu� donare SOLO a strutture pubbliche
	...
	public function execute_action ( $par, &$message ) 
	{
		...
		// se la struttura target � il dump, e 
		...
		if ( $par[0] -> structure_type -> type != 'dump' )
		{
			$text = '__events.structuredonation;'. $par[3] -> name . ';' . $par[2] . ';' . '__' . $par[1] -> cfgitem -> name ;
			 Model_StructureEvent::newadd( $par[0] -> id, $text );
		}
		...
		Model_CharacterEvent::addrecord( $par[3] -> id, 'normal', $text );
