...
class Model_CharacterAction_CAMove extends Model_CharacterAction
{
	...
	{ 
		...
		$regionpaths = Model_Configuration::get_cfg_regions_paths2();
		...
		// Controllo che il char non sia gi� nella stessa locazione
		...
		if ( 
			...
			Model_RegionPath::ispathcrossable($par[2], $currentregion, $region_path ) == false)
		{ $message = kohana::lang('ca_move.error-pathisnotcrossable'); return FALSE; }
		
		
		// se il peso in eccesso � del x% errore
		...
		*/
		...
		$travelinfo = Model_RegionPath::get_travelinfo( $this -> _par );
		...
		/////////////////////////////////////////////////////
		// Controllo che se movetobattlefield � true, nella 
		...
		/////////////////////////////////////////////////////
		// controlla se il char non pu� lasciare il regno
		// ordine di restraint
		/////////////////////////////////////////////////////
		...
		$possibletomove = Model_Region::canmoveto( $par[2], $par[0], $currentregion, $par[1], $message );
...
//		var_dump($travelinfo); exit;
		...
		// se � in un battlefield, lo tolgo automaticamente
		...
		if ( Model_Character::is_fighting( $par[2] -> id ) == true )
		{
			...
			Model_CharacterEvent::addrecord( $par[2] -> id , 'normal', '__events.battlefield_leave');
	...
	public function complete_action( $data )
	{
		...
		Model_Item::consumeclothes( $char, $this -> action, $data -> param4 );
		...
		////////////////////////////////////////////////
		// se lo stato � fighting, vuol dire che il char
		...
		// evento per quest
		$par[0] = $destregion;
		Model_GameEvent::process_event( $char, 'travel', $par );

		
		Model_CharacterEvent::addrecord( $char -> id , 'normal',
			'__events.move_end' . 			
			';__' . $destregion -> name );
	...
	{		
		...
		// lo rimetto nel battlefield. Se non c'� il battlefield, 
