...
class Character_Model extends ORM
{
	...
	*/
	...
	function get_restfactor( $structure, $freerest = false, $cartrest = false )
	{
		...
		kohana::log('info', '-> Timeforfullenergy: [' . $info['timeforfullenergy'] . '] - ' . 
			'[' .Model_Utility::secs2hmstostring( $info['timeforfullenergy'] ) .']' );
		...
		kohana::log('info', '-> Timeforfullenergy: [' . $info['timeforfullenergy'] . '] - [' . Model_Utility::secs2hmstostring( $info['timeforfullenergy'], 'hours' ) . ']' );
	...
	*/
	...
	function modify_energy( $delta, $assigndirectly = false, $reason = 'notspecified' )
	{
		...
		Character_Event_Model::addrecord ( 
			...
			';' . (Model_Utility::number_format($delta/50,2)*100) .
	...
	**/
	...
	function deletecharfromdb()
	{
		...
		$result = Model_Utility::send_notification( $this -> user -> id, $subject, $body );
