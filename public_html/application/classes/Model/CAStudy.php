...
class Model_CharacterAction_CAStudy extends Model_CharacterAction
{
	// Saziet� richiesta per ogni ora di studio
	...
	{ 
		...
		// Check: il char non ha sufficiente saziet�
		if
		( 
		...
		$rd = Model_DiplomacyRelation::get_diplomacy_relation( $par[1] -> region -> kingdom_id, $par[0] -> region -> kingdom_id );
		...
		$this -> course = Model_CourseFactory::create($par[3]);
		...
		// controllo se il char ha gi� troppi skilsl
		if ( 
			$this -> course -> getCoursetype() == 'skill' 
			and
			Model_Skill::get_character_skillcount( $par[0] -> id ) >= 3
		)
		{
		...
		// controllo che non abbia gi� masterizzato il corso
	...
	protected function append_action( $par, &$message )	{	
		...
		Model_StructureEvent::newadd( $par[1] -> id, $text );
		...
		if ($par[1] -> structure_type -> supertype == 'academy' )
		{
			$itemtoremove = Model_Item::factory( null, $this -> supportitem);
			$itemtoremove -> removeitem( 'structure', $par[1] -> id, $par[2]);
		}
		else
			Model_Item::consumeitem_instructure( $this -> supportitem, $par[1] -> id, $this -> consumerate * $par[2] );
	...
	public function complete_action( $data )	
	{
		...
		$this -> course = Model_CourseFactory::create($data -> param2);
		...
		Model_Item::consume_equipment( $this -> equipment, $character, $data -> param1 );
		...
		// Se il corso � completato, si aumenta la caratteristica
