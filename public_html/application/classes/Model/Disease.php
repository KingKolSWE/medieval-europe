...
abstract class Model_Disease
{
	...
	protected $strmalus;
	protected $dexmalus;	
	protected $iscurable;  			// � curabile?
	protected $intelmalus;	
	protected $costmalus;
	protected $carmalus;
	protected $iscyclic;					// � ciclica? Se sì viene instanziata una azione ciclica
	protected $isblocking;
	protected $timedependent;
	protected $cooldown;         	// Se si ha la malattia meno di $cooldown giorni fa non si può riprendere
	protected $requireditem;     	// Oggetto richiesto per curare il char (tag)
	protected $timetocure;       	// Tempo base necessario a curare (ore)
	
	/*
	* Applica la malattia
	* @param obj $char Character_Model personaggio a cui la malattia � applicata
	*/
	...
	*/
	...
	{	
		...
		if ( $this -> get_iscyclic() == true )
		{
			...
			$action = new Model_CharacterAction();
		...
		// Se � blocking, fa collassare il char
	...
	/*
	* torna se la malattia � attiva oppure no
	...
	*/
	...
	{	
		...
		// Se la malattia non � infettiva... no action
		...
		foreach ( (array) $charstoinfect as $chartoinfect )
		{
			...
			// se � già infettato non può essere reinfettato. 
			...
			// se � stato curato ed ha avuto la malattia non più di cooldown giorni fa
			// giorni fa non può prenderla ancora
			...
			// se � in meditazione non può essere infettato			
			...
			// se ha meno di 30 giorni non può essere infettato
			...
			// se si sta muovendo non � possibile infettarlo
