...
class Model_Region extends ORM
{
	...
	*/
	...
	public function get_terrains_info( $region )
	{
		...
		$terrains_info = array();
		$terrains_info['max_terrains_x_region'] = Model_Structure_STTerrain::get_maxfield_x_region ( $region );
	...
	*/
	...
	public function get_regions_info()
	{
		...
		$regionsinfo = Model_MyCache::get( $cachetag );
		...
		if ( is_null( $regionsinfo ) )
		{
			...
			Model_MyCache::set( $cachetag, $regionsinfo );
	...
	*/
	...
	public function get_structure( $supertype )
	{
		...
		$structure = Model_StructureFactory::create( null, $structure[0] -> id );
	...
	* @return Model_CharacterRole personaggio nel ruolo o NULL se non trovato.
	*/
	...
	*/
	...
	public function get_controllingroyalpalace()
	{
		...
		$capitalregion = Model_Kingdom::get_capitalregion($this -> kingdom_id);
	...
	public function get_charinrole( $roletag )
	{
		...
		else
		{
			$controlledstructures =
				Model_CharacterRole::get_controlledstructurestags( $roletag );
	...
	*/
	...
	public function get_appliable_tax( $region, $name, $character )
	{
		...
		$dr = Model_DiplomacyRelation::get_diplomacy_relation(
			$region -> kingdom_id,
	...
	*/
	...
	public function get_structures( $supertype )
	{
		...
		foreach ( $structures as $structure )
			$structures_a[] = Model_StructureFactory::create( null, $structure -> id );
	...
	*/
	...
	public function get_allpaths()
	{
		...
		$cachetag = '-allpaths' ;
		$allpaths = Model_MyCache::get( $cachetag );
		...
		if ( is_null( $allpaths ) )
		{
			...
			Model_MyCache::set( $cachetag, $allpaths );
	...
	* @param Model_Region $sourceregion Regione da cui parte il char.
	...
	*/
	...
	function find_adjacentregions( $sourceregion )
	{
		...
		require_once( dirname(dirname(__FILE__)) . "/libraries/vendors/Dijkstra/Dijkstra.php");
		$g = new Graph();
		$regionpaths = Model_Configuration::get_cfg_regions_paths();
		$regions = Model_Configuration::get_cfg_regions();
	...
	*/
	...
	function move( $targetkingdom )
	{
		...
		$allstructures = Model_Region::get_structures_d( $this -> id );
			...
			foreach ( $structures as $structure )
			{
					...
					'buildingsite' ) ) )
				{
					...
					else
					{
						...
						{
							$buildingstructure = Model_Structure_STBuildingsite1::get_info( $structure -> id );
		...
		if ( $isonwar !== false )
		{
			...
			foreach ( $data['battles'] as $battle )
			{
				...
				if ( $attackedregion -> kingdom_id  == $targetregion -> kingdom_id )
				{
					...
					$dr = Model_DiplomacyRelation::get_diplomacy_relation( $char -> region -> kingdom_id,
