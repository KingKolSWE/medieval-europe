...
class Model_CharacterAction_CATake extends Model_CharacterAction
{
	...
	{ 
		...
		// check: la struttura esiste ed il char � proprietario?
		...
		// check: l' oggetto � nella struttura, la quantit� � corretta?		
		if ( ! $par[1] -> loaded or
	...
	public function execute_action ( $par, &$message ) 
	{
		...
		// evento per quest			
		$_par[0] = $par[1];
		Model_GameEvent::process_event( $par[3], 'takeitemsfrominventory', $_par );
		...
		$text = '__events.structuretake;'. $par[3]->name . ';' . $par[2] . ';' . '__' . $par[1]->cfgitem->name ;
		 Model_StructureEvent::newadd( $par[0], $text );
