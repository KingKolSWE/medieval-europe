...
function get_wardrobeitemdata( $name )
{
	...
	$cfg = Model_Configuration::get_wardrobeitemcfg();
	return $cfg[$name];
...
)
{
	...
	$char -> sex;
	//filename dell' immagine da visualizzare se la cust.ne non � stata caricata
	$nocustomimage = 'media/images/template/noimage.png';
	...
	while ( $i <= 5 )
	{
		...
		if ( $i == 0 ) 
		{
			...
			if ( $i == $stat -> value )
				$html .= "<span style='color:#cc0000'>" . Model_Utility::truncateHtml(
						kohana::lang($name), 15, '...', true, false ) . "* </span>";
			else
				$html .= Model_Utility::truncateHtml(
		...
		else
		{
			...
			// altrimenti controllo se c'� una immagine in 
	...
	)
{
	...
	$cfg = Model_Configuration::get_wardrobeitemcfg();
	$cfgbonuses = Model_Configuration::get_premiumbonuses_cfg();
	//var_dump($cfgbonuses);
	...
	<div class='atelieritemwrapper'>
		...
		<b> " . Model_Utility::truncateHtml(kohana::lang($description), 30) . "</b>" . "</div>" .
...
function get_correctimage( $char, $equippeditem, $mode, $itemtype = 'item' )
{
	...
	if ( $mode == 'wardrobe' )
	{
		...
		elseif ( $itemtype == 'item')
		{
			...
			// se lo slot non esiste o � 0, carichiamo l' immagine di default.
			if ( !$slotstat -> loaded or $slotstat -> value == 0 )
			{
				// se il file � colorato, overrida la immagine.						
	...
	elseif ( $mode == 'preview' )	
	{
		...
		elseif ( $itemtype == 'item' )
		{
			...
			// vediamo se c'� una immagine in attesa di approvazione (� nella dir. temp...)		
