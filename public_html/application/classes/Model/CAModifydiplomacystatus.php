...
class Model_CharacterAction_CAModifydiplomacystatus extends Model_CharacterAction
{	
	...
	// $par[1] = oggetto structure da cui � invocata l' azione
	...
	{ 
		...
		// controllo: il nuovo stato � diverso da quello precedente?
		
		if ( $par[3] == $par[2] -> type  )
		{ $message = kohana::lang('diplomacy.error-statusmustbedifferent'); return FALSE; }									
		// controllo: se non � una conferma, se c'� gi� una proposta inviata, errore
		...
		// Non pi� di due Alleati.
		
		$alliedkingdoms = 0;
		$diplomacyrelations = Model_Configuration::get_cfg_diplomacyrelations();
		...
		$lastwarsource = Model_Kingdom::get_last_war( $par[2] -> sourcekingdom_id );
		if (
		...
		$kingdomrunningwars = Model_Kingdom::get_kingdomwars( $par[0] -> region -> kingdom_id, 'running' );
		
		// Se il kingdom sorgente � in guerra non � possibile cambiare relazioni diplomatiche
	...
	public function execute_action( $par, &$message )
	{
		...
		$paper_piece = Model_Item::factory( null, 'paper_piece' );
		...
		// se la modalit� � diretta o � una conferma, setta stato diplomatico.
		...
		if ($requirement == 'direct' or ($par[5] == true and $par[4] == 'accept'))
		{
			...
			// se il re destinatario � assente il trattato � firmato unilateralmente dal re sorgente.
			...
			$runningwars = Model_Kingdom::get_kingdomwars( $sourcekingdom -> id, $status = 'running' );
			
			// Se la proposta � != allied e il regno proponente � 
			...
			if ( $proposal -> diplomacyproposal != 'allied' )
			{
				...
				if ( count( $runningwars ) > 0 )
				{
					...
					$kingdomwar_ally = ORM::factory('Model_KingdomWarsAlly')
						-> where( array(
			...
			// Se la proposta � di alleanza, e il regno proponente �
			...
			if ( $proposal -> diplomacyproposal == 'allied' )
			{
				...
				if ( count( $runningwars ) > 0 )
				{
					...
					$kingdomwars_allies = new Model_KingdomWarsAlly();
		...
		else
		{
			...
			if (!is_null($targetking))
			{
				...
				$proposal = new Model_DiplomacyProposal();
