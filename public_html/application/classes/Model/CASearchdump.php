...
class Model_CharacterAction_CASearchdump extends Model_CharacterAction
{
	// Costanti	
	const DELTA_GLUT = 3;
	...
	public function complete_action( $data )
	{
		...
		if ( $founditem )
			Model_CharacterEvent::addrecord( $char -> id, 'normal', '__events.dumpobjectfound' .
			';' . $itemfound -> quantity . 
			';__' . $itemfound -> name, 
			'evidence' );				
		else
			Model_CharacterEvent::addrecord( $char -> id, 'normal', '__events.dumpobjectfoundnothing' );
