...
class Model_CharacterAction_CAAssignrole extends Model_CharacterAction
{
	...
	*/
	...
	{ 
		...
		$structuretags = Model_CharacterRole::get_controlledstructurestags( $par[2] );
		...
		{
			$this -> required_fp = Model_CharacterRole::get_requiredfp( $par[1], 'assign', $par[2] );
		...
		{
			$this -> required_sc = Model_CharacterRole::get_requiredcoins( $par[1], 'assign', $par[2] );
		...
		if ( Model_CharacterRole::check_eligibility( $par[1], $par[2], $par[0] -> church, $message ) == false )
		{						
			return false;
		}
	...
	{	
		...
		Model_CharacterRole::start( $par[1], $par[2], $par[3], $par[4] -> id, null, false );
