...
class Model_CharacterAction_CAInitiate extends Model_CharacterAction
{
	// Costanti	
	const DELTA_GLUT = 5;
	...
	*/
	...
	{ 
		...
		// Check: qualche parametro non � caricato
		...
		// Check: il char � scomunicato (non pu� essere battezzato per 3 mesi)
		...
		$pendingaction = Model_CharacterAction::get_pending_action( $par[1] -> id );
		...
		// Check: il char che esegue l'iniziazione non ha energia o saniet� sufficienti
		...
		$fp = Model_Structure::get_stat_d( $par[2] -> id, 'faithpoints' );
	...
	*/
	protected function append_action( $par, &$message )
	{
		...
		Model_MyCache::delete(  '-charinfo_' . $a -> character_id . '_currentpendingaction');
		...
		Model_Item::consumeitem_instructure( 'goldenbasin', $par[2] -> id, self::GOLDENBASIN_WEAR );
	...
	public function complete_action( $data )
	{
		...
		// Struttura religiosa
		$structure = Model_StructureFactory::create( null, $data -> param3 );
		...
		if ( $charaction -> id == $chartarget -> id )
		{
			...
			Model_CharacterPermanentevent::add_model( $chartarget -> id,
	...
	{
		$info = Model_Church::get_info($par[2] -> structure_type -> church_id);
