...
class Model_CharacterAction_CAcancelMarriage extends Model_CharacterAction
{
	// Costanti	
	const DELTA_GLUT = 5;
	...
	protected function check( $par, &$message )
	{
		...
		$data = Model_CharacterRelationship::get_kinrelations( $par[2] -> id );
	...
	public function complete_action( $data )
	{
		...
		if ( $charaction -> id == $officer -> id )
		{
			...
			Model_Item::consume_equipment( $this->equipment, $charaction );
		...
		if ( $charaction -> id == $requester -> id )
		{
			...
			if ( $requester -> sex == 'M' )	
			{
				Model_CharacterRelationship::remove_model( $requester -> id, $partner -> id, 'husband' );
				Model_CharacterRelationship::remove_model( $partner -> id, $requester -> id,
				'wife');
			}
			else
			{
				Model_CharacterRelationship::remove_model( $requester -> id, $partner -> id, 'wife' );
				Model_CharacterRelationship::remove_model( $partner -> id, $requester -> id, 'husband' );
			}
						
			// events, annullment completed.
			
			Model_CharacterEvent::addrecord( $requester -> id,
			...
			Model_CharacterEvent::addrecord( $partner -> id,
			'normal', '__events.cancelweddingspouses' . 
			';' . Model_Character::create_publicprofilelink($requester -> id, $requester -> name) .
			';__' . $structure -> structure_type -> name .
			';__' . $structure -> region -> name,
			...
			Model_CharacterPermanentevent::add_model( $requester -> id,
			...
			Model_CharacterPermanentevent::add_model( $partner -> id,
			'__permanentevents.marriedcanceled' .
			';' . Model_Character::create_publicprofilelink($requester -> id, $requester -> name) .
