...
class Model_Structure extends ORM
{
	...
	public function build_common_links( $structure, $workerbonus = false )
	{
		...
		if ( !is_null($structure -> status) and $structure -> status == 'upgrading' )
		{
			...
			$links .= kohana::lang('structures.overallprogress') . ": " . $workedhours . "/" . $upgradeinfo['hours'] . " ("
				. Model_Utility::number_format($workedhours/$upgradeinfo['hours']*100,2). "%)<br/>";
	...
	*/
	...
	public function modify_coins( $delta, $reason = 'notspecified', $sendevent = true )
	{
		...
		Model_TraceSink::add_model( 'silvercoin', $this -> id, round($delta, 2), $reason, 'structure' );
...
*/
	...
	function allowedaccess( $char, $parenttype = 'notset', &$message, $accesstype = 'private', $action = null )
	{
		...
		if ( $accesstype == 'private' )
		{
			...
			$grants = Model_StructureGrant::get_chargrants( $this, $char );
			...
			)
			{
				...
				if ($ownedshops > Kohana::config('medeur.maxshops'))
				{
					...
					$churchhasoraetlaborabonus = Model_Church::has_dogma_bonus( $char -> church_id, 'craftblessing');
	...
	*/
	...
	function get_info()
	{
		...
		$faithpoints = Model_Structure::get_stat_d( $this -> id, 'faithpoints' );
	...
	*/
	...
	function transfer_ownership( $seller, $buyer )
	{
		...
		if ( ! is_null( $vassal ) )
		{
			...
			Model_CharacterEvent::addrecord( $vassal->id,
	...
	*/
	...
	public function modify_fp( $delta, $reason = '' )
	{
		...
		$fp = Model_Structure::get_stat_d( $this -> id, 'faithpoints' );
  ...
  * @param replace: se true rimpiazza il valore (value) e non lo somma/sottrae
	...
	*/
		...
		$replace = false )
	{
		...
		$stat = Model_Structure::get_stat_d( $this -> id, $name, $searchparam1, $searchparam2 );
  ...
  * @param replace: se true rimpiazza il valore (value) e non lo somma/sottrae
	...
	*/
		...
		$replace = false )
	{
		...
		$stat = Model_Structure::get_stat_d( $structure_id, $name, $searchparam1, $searchparam2 );
	...
	*/
	...
	public function set_option( $name, $value )
	{
		...
		if ( $found == false )
		{
			$option = new Model_StructureOption();
			$option -> name = $name;
	...
	*/
	...
	public function change_description()
	{
		...
		$structure = Model_StructureFactory::create( null, $this -> input -> post( 'structure_id' ) );
	...
	*/
	...
	function getDescription()
	{
		if( empty($this -> description) )
			...
			return Model_Utility::bbcode($this -> description);
	}
	...
	*/
	...
	public function getUpgradeworkedhours()
	{
		...
		$stat = Model_Structure::get_stat_d(  $this -> id, 'upgradeworkedhours' );
		if ($stat -> loaded )
				return $stat -> value;
		else
				return 0;
	...
	*/
	...
	function getAvailablecourses()
	{
		...
		$installedcourses = Model_Structure::get_stats('course');
