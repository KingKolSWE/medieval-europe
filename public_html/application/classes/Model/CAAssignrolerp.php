...
class Model_CharacterAction_CAAssignrolerp extends Model_CharacterAction
{
	...
	// @param   par[1]: oggetto char di chi � nominato
	...
	{ 
		...
		// Il char pu� nominare solo alcuni ruoli rp
		...
		// Controllo che il char non abbia gi� il ruolo
	...
	{			
		...
		Model_Item::consume_equipment( $this->equipment, $par[0]);
		
		///////////////////////////////////////////
		// Creo il ruolo RP;
		///////////////////////////////////////////
		
		Model_CharacterRole::start( $par[1], $par[2], $par[3], $par[4]->id, $par[5], true );
		...
		$knt = new Model_KingdomNobletitle;
		$customtitles = $knt->get_customisedtitles($par[0] -> region -> kingdom ->id);
		$temprole='';
		// Verifico se il titolo assegnato � customizzato
		...
		// se il titolo � cancelliere, d� le grant dovute
		if ( $par[2] == 'chancellor' )
		{
			$royalpalace = $par[4] -> region -> get_controllingroyalpalace();
			$ca = new Model_CharacterAction_CAAssignstructuregrant();
			$_par[0] = $royalpalace;
			$_par[1] = $par[1];
			$_par[2] = $par[2];
		...
		Model_CharacterEvent::addrecord(	null, 'announcement', $msg1	);
		
		// Pubblica annuncio negli eventi del char nominato
		Model_CharacterEvent::addrecord( $par[1] -> id, 'normal', $msg2 );
