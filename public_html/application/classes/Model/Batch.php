...
class Model_Batch
{
	...
	public function test_email() {
		Model_Utility::mail( kohana::config('medeur.adminemail') , "Medieval Europe [Server {$config['servername']}] - Testing.", 'this is a test');
		echo 'sent';
	}
	...
	*/
	...
	public function watchdog($securitykey = null)
	{
		...
		if ( $count  > 0 )
			Model_Utility::mail( kohana::config('medeur.adminemail') , "Medieval Europe [Server {$config['servername']}] - Found {$count} blocked actions.", $body);
	}
	...
	*/
	...
	function sendstarvingemail($securitykey=null)
	{
		...
		foreach ( $rset as $row )
		{
			...
			Model_Utility::send_notification( $row -> id, $subject, $body );
    ...
    and
		...
		if ( count ( $rset ) > 0 )
			foreach ( $rset as $r )
			{
				...
				. Model_Utility::secs2hmstostring( $r -> secs, 'hours' )
				...
				Model_Utility::send_notification( $r -> user_id, $subject, $body );
