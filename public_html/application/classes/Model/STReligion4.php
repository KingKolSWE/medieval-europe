...
class Model_Structure_STReligion4 extends Model_Structure
{
	public function init()
	{		
		$this -> setCurrentLevel(1);
		$this -> setParenttype('religion_4');
		$this -> setSupertype('religion_4');
		$this -> setMaxlevel(1);
		$this -> setIsbuyable(false);
		$this -> setIssellable(false);		
		$this -> setStorage(10000000);	
		$this -> setWikilink('En_US_Religious_Structure_level_4');				
	}
	...
	public function build_common_links( $structure, $bonus = false )
	{
		...
		$links .= kohana::lang('global.condition') . ': '. $structure -> display_condition( ) . '<br>';
		...
		$links .= "<br/>";
		$links .= html::anchor( "/religion_4/donatecoins/" . $structure -> id, Kohana::lang('religion.donatecoins'), 
			array('class' => 'st_common_command',
			'onclick' => 'return confirm(\''.kohana::lang('structures_religion_4.confirm_donation').'\')'
			)) . "<br/>";
