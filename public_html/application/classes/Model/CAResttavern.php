...
class Model_CharacterAction_CAResttavern extends Model_CharacterAction
{
	...
	protected function check( $par, &$message )	
	{
		...
		$rd = Model_DiplomacyRelation::get_diplomacy_relation( $par[2] -> region -> kingdom_id, $par[0] -> region -> kingdom_id );
		...
		// check: se la saziet� � a 0, il char non puo' riposare
	...
	protected function append_action( $par, &$message )
	{
		...
		// se il prezzo non � free, applica tasse
		...
		Model_CharacterEvent::addrecord( $par[0] -> id, 'normal',
			'__events.resttavernstartnew' . 
			';' . $this -> totalprice . 
			';' . Model_Utility::secs2hmstostring( $timetorest, 'hours' ) .
			';' . ($par[5])
		) ;
	...
	public function complete_action( $data )
	{
		...
		Model_GameEvent::process_event( $character, 'resttavern', $_par );
	...
	{	
		...
		// Energia = Fattore di riposo memorizzato quando si � iniziata 
		...
		Model_CharacterEvent::addrecord( $character -> id,
			'normal', 
			'__events.restcancel'
		);			
