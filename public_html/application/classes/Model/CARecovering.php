...
class Model_CharacterAction_CARecovering extends Model_CharacterAction
{
	// Costanti
	const DELTA_ENERGY = 0;        // Energia necessaria per la semina
	const DELTA_GLUT = 0; 		   // consumo di saziet�	
	...
	protected $cancel_flag = false; // se true, la azione � cancellabile dal pg.	
	...
	public function complete_action( $data )
	{
		...
		if ( $data -> param1 != 'duel' )
		{
			...
			// Lancio per ferite sanguinanti. Le probabilit� vanno
			...
			if ( $r <= $chance )
			{
				...
				$disease = Model_DiseaseFactory::createDisease('bleedingwound');
