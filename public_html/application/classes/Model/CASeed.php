...
class Model_CharacterAction_CASeed extends Model_CharacterAction
{
	// Costanti
	const DELTA_GLUT = 7; 					// consumo di saziet�	
	const DELTA_ENERGY = 8;         // Energia necessaria per la semina		
	const TIME_TO_GROW = 36000 ;    // Tempo necessario per la crescita

	protected $cancel_flag = true;     // se true, la azione � cancellabile dal pg.	
	...
	protected function append_action( $par, &$message )
	{
		...
		$fertilizer = Model_Item::factory( null, 'fertilizer');
	...
	public function cancel_action() {
		...
		if ( Model_StructureGrant::get_chargrant( $structure, $character, 'workerpackage' ) == true )
			return false;
			
		Model_StructureEvent::newadd( $this -> param1,
	...
	public function complete_action( $data )
	{
		...
		$terrain = Model_StructureFactory::create('terrain', $data -> param1);
		...
		// Sottraggo l'energia e 1 punto di saziet� al char
		...
		$message=null;
		$a = Model_CharacterAction::factory("growfield");
		...
		// evento per quest
		$par[0] = $typeseed;
		Model_GameEvent::process_event( $char, 'seedfield', $par );
		...
		if ( $char -> id != $terrain -> character_id )
			Model_Job::givehourlywage( $terrain, $char, $this -> basetime );
