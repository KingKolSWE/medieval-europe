...
class Model_CharacterAction_CA_CelebrateMarriage extends Model_CharacterAction
{
	// Costanti	
	const DELTA_GLUT = 5;
	...
	protected function check( $par, &$message )
	{
		...
		$pendingaction = Model_CharacterAction::get_pending_action( $par[2] -> id );
		
		if ( !is_null( $pendingaction ) )		
		{ $message = Kohana::lang("global.error-characterisbusy", $par[2] -> name); return false; }				
		$pendingaction = Model_CharacterAction::get_pending_action( $par[3] -> id );
		if ( !is_null( $pendingaction ) )		
		...
		$fp = Model_Structure::get_stat_d( $par[1] -> id, 'faithpoints' );
		...
		// C'� almeno un golden basin al 25%?
		$exists = false;
	...
	protected function append_action( $par, &$message )
	{
		...
		Model_Item::consumeitem_instructure( 'goldenbasin', $par[1] -> id, self::GOLDENBASIN_WEAR );
				
		$item = Model_Item::factory(null, 'flowers');
		$item -> removeitem( 'structure', $par[1] -> id, 100 );
		
		$item = Model_Item::factory(null, 'mulberry_cake');
		$item -> removeitem( 'structure', $par[1] -> id, 20 );
		
		$item = Model_Item::factory(null, 'winebottle');
		$item -> removeitem( 'structure', $par[1] -> id, 20 );
		
		$item = Model_Item::factory(null, 'beerbottle');
		$item -> removeitem( 'structure', $par[1] -> id, 10 );
		
		$item = Model_Item::factory(null, 'mulberrybrandybottle');
		$item -> removeitem( 'structure', $par[1] -> id, 5 );
		
		$item = Model_Item::factory(null, 'meadbottle');
		$item -> removeitem( 'structure', $par[1] -> id, 5 );
	...
	public function complete_action( $data )
	{
		...
		$structure = Model_StructureFactory::create( null, $data -> param4 );
		...
		if ( $charaction -> id == $officer -> id )
		{
			...
			Model_Item::consume_equipment( $this->equipment, $officer );
		
			// rendo la proposta non pi� valida.
			...
			Model_CharacterEvent::addrecord( $officer -> id,
			...
			Model_CharacterEvent::addrecord( null,
		...
		// Azioni relative a chi ha sub�to la funzione
		// 1. Setta relationship
		
		if ( $charaction -> id == $husband -> id )
		{		
			Model_CharacterRelationship::add_model( $charaction -> id, $wife -> id, 'husband' );
			Model_CharacterEvent::addrecord(
				$husband -> id , 
				'normal', 
				'__events.celebratemarriage_end' );				
			...
			Model_CharacterPermanentevent::add_model( $husband -> id,
		...
		{		
			Model_CharacterRelationship::add_model( $charaction -> id, $husband -> id, 'wife' );
			Model_CharacterEvent::addrecord(
				$wife -> id , 
				'normal', 
				'__events.celebratemarriage_end' );				
			
			Model_CharacterEvent::addrecord( $wife -> id,
			'normal', 
			'__events.celebrateweddingspouses' . 
			';' . Model_Character::create_publicprofilelink($husband -> id, $husband -> name) .
			';__' . $structure -> structure_type -> name .
			...
			Model_CharacterPermanentevent::add_model( $wife -> id,
			'__permanentevents.married' .
			';' . Model_Character::create_publicprofilelink($husband -> id, $husband -> name) .
			';__' . $structure -> structure_type -> name .
			';__' . $structure -> region -> name			
	...
	protected function get_neededfp( $par )
	{
		...
		$info = Model_Church::get_info($par[1] -> structure_type -> church_id);
