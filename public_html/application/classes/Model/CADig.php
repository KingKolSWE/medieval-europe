...
class Model_CharacterAction_CADig extends Model_CharacterAction
{
	// Costanti
	const DELTA_GLUT = 6;
	...
	*/
	...
	{ 
		...
		// Check: la regione � indipendente (l'accesso alla risorsa � interdetto)
		if ( $par[0] -> region -> is_independent() )
		{ $message = Kohana::lang("charactions.regionisindependent"); return false; }
	
		// Check: la relazione tra il regno del char e quella del regno della regione � ostile o neutrale
		$rd = Model_DiplomacyRelation::get_diplomacy_relation( $par[0] -> region -> kingdom_id, $par[1] -> region -> kingdom_id );
		...
		// Check: il moltiplicatore non � nel range 1-3
		// Check: il moltiplicatore � > 1 e il char non ha il relativo bonus		
		$queuebonus = false;
	...
	protected function append_action( $par, &$message )
	{
		...
		//var_dump( $items); exit;
		$param2 = '';		
		// toglie la quantit� alle risorse
		foreach ( $items as $key => $value )
		{
			foreach ( $par[0] -> structure_resource as $resource )
				if ( $resource -> resource == $key )
	...
	*/
	...
	/*
	* Prevede quante unit� estrarra in base alle caratteristiche
	...
	*/
	...
	private function computeproduction( $par )
	{
		...
		$item = Model_Item::factory( null, $resourcetype );
	...
	public function complete_action( $data )
	{
		...
		Model_Item::consume_equipment( $this->equipment, $char, $data -> param3 );
		...
		foreach ( $items as $item )
		{
			...
			if ( $quantity > 0 )
			{
				$item = Model_Item::factory( null, $itemtag );
	...
	{								$structure = Model_StructureFactory::create( null, $this -> param1 );
