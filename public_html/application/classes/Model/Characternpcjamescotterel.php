...
class Model_JamesCotterelCharacterNPC extends Model_CharacterNPC
{
	...
	{		
		...
		// 2. se � sotto 30 di saziet�, compra cibo nel mercato. Se non trova il mercato
		// si sposta.
		...
		if ($this -> glut < 30 )
		{
			...
			// trovo cibo nel mercato, ordinato dal pi� cheap.
	...
	public function die_aftermath()
	{
		...
		if ($item['quantity'] > 0)
		{
			...
			$droppeditem = Model_Item::factory(null, $item['tag']);
			$droppeditem -> additem("region", $this -> getPosition_id(), $item['quantity']);			
		}

		// evento per quest		
		$char = Model_Character::get_info( Session::instance()->get('char_id') );
		Model_GameEvent::process_event( $char, 'killrat', null );
