...
class Model_CharacterAction_CASenditem extends Model_CharacterAction
{
	// Costanti	
	const DELTA_GLUT = 0;
	...
	{ 
		...
		$info = Model_Item::computesenddata( $par[2], $par[3] -> id, $par[0], $par[1] -> name , 'send' );
	...
	protected function append_action( $par, &$message )
	{
		...
		///////////////////////////////////////////////////////////		
		// Aggiungiamo la quantit� dell' oggetto ad un fantomatico 
		// char -1
		///////////////////////////////////////////////////////////		
		...
		// togliamo la quantit� di risorse specificata al momento del send
		// se non sono silvercoin, altrimenti � incluso nel costo...
		...
		kohana::log('debug', 'ca_senditem: append sender: ' . $par[0]->id . ' receiver: ' . $par[1]->id . ' time: ' .  Model_Utility::format_datetime($this -> info['time']) );
		...
		Model_CharacterEvent::addrecord(
			...
			';' . $par[1]-> name .
			';' . Model_Utility::format_datetime( $this -> endtime )
		);
	...
	{ 
		...
		// se il char nel frattempo muore, l' item � perso per sempre
		...
		if ( $item -> loaded )
		{
			if ( $receiver -> loaded )
			{
				...
				$par[1] = $sender;				
				$par[2] = $receiver;				
				Model_GameEvent::process_event( $receiver, 'receivedquesttoken', $par );
