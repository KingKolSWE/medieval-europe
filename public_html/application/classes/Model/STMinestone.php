...
class Model_Structure_STMinestone extends Model_Structure
{
	...
	public function build_common_links( $structure, $bonus = false )
	{
		...
		$links .= html::anchor( "/mine_stone/dig/" . $structure->id, Kohana::lang('structures_actions.mine_dig'), array( 'class' => 'st_common_command',
			'onclick' => 'return confirm(\''.kohana::lang('global.confirm_operation').'\')' ));

		if ( $bonus !== false )
		{
		   	$links .= ' - '.html::anchor( "/mine_stone/dig/".$structure->id."/2", 'x2',
		    array('title' => Kohana::lang('structures_actions.mine_dig').' (x2)', 'class' => 'st_common_command',
					'onclick' => 'return confirm(\''.kohana::lang('global.confirm_operation').'\')' ));
		    
		   	$links .= ' - '.html::anchor( "/mine_stone/dig/".$structure->id."/3", 'x3',
