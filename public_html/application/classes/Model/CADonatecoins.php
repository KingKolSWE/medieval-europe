...
class Model_CharacterAction_CADonatecoins extends Model_CharacterAction
{
	...
	{ 
		...
		// se si ha un ruolo nella chiesa, non si pu� donare
	...
	public function execute_action ( $par, &$message ) 
	{
		...
		Model_CharacterEvent::addrecord( $par[0] -> id, 'normal', '__events.donatecoins' . ';' .
		...
		// d� i soldi alla struttura livello 4
		//$coins -> additem( 'structure', $par[1] -> id, 4 );	
		
		$par[1] -> modify_coins( 4, 'donatecoins' ); 
		
		Model_StructureEvent::newadd( $par[1] -> id,
		...
		if ( $p1structure -> loaded )
		{
			...
			Model_StructureEvent::newadd( $p1structure -> id,
			'__events.structure_donatedcoins' . ';' .				
				$par[0] -> name . ';' . 
				3);
			...
			if ( $p2structure -> loaded )
			{
				...
				// evento
				Model_StructureEvent::newadd( $p2structure -> id ,
				'__events.structure_donatedcoins' . ';' .				
				$par[0] -> name . ';' . 
				2);
