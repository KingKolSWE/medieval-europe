...
class Model_KingdomForumTopic extends Model_Topic
{				
	...
	function read( $char, $topic_id, &$topic )
	{
		...
		if ( Model_KingdomForumTopic::haswriterights( $char, $topic -> kingdom_forum_board -> kingdom ) == false )
		{
	...
	function add_model($char, $data, &$message )
	{
		...
		if ( Model_KingdomForumTopic::haswriterights( $char, $currentboard -> kingdom ) == false )
		{
			$message = 'global.operationnotallowed';
			return false;
		}
